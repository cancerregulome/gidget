...
if [[ $# != 5 ]] && [[ $# != 6 ]]
    then
        ...
        echo " Example : `basename $0` 28oct13  dcc-snapshot-28oct13  brca  parse_tcga.all450k.config  private  aux"
        echo " "
...
$TCGAFMP_ROOT_DIR/shscript/fmp01B_xml_refactor.sh $curDate $snapshotName $oneTumor $config $ppString $auxName >& $TCGAFMP_DATA_DIR/$oneTumor/scratch/fmp01B.$curDate.$snapshotName.$oneTumor.log
...
$TCGAFMP_ROOT_DIR/shscript/fmp05B_filter_450.sh $curDate $oneTumor $auxName >& $TCGAFMP_DATA_DIR/$oneTumor/scratch/fmp05B.$curDate.$oneTumor.log
