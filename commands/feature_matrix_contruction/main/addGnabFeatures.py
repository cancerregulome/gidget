...
def readPathways ():
    ...
    fh = file ( "/proj/ilyalab/sreynold/TCGA/pathways/only_NCI_Nature_ver4.tab", 'r' )
...
def pathwayGnab ( dataD, pathways={} ):
    ...
    if ( len(pathways) == 0 ):
        ...
        pathways["TP53_pathway"] = [ "E2F1", "TP53", "RB1", "CDK4", "TIMP3", "CDK2", "ATM", 
		       "CCNE1", "CCND1", "CDKN1A", "BCL2", "BAX", "PCNA", "MDM2", 
		       "APAF1", "GADD45A" ] 

        pathways["PI3K_AKT_pathway"] = [ "FRAP1", "LST8", "PDPK1", "NGF", "NR4A1", "FOXO1", "CHUK", 
    		       "THEM4", "PTEN", "CREB1", "BAD", "RHOA", "TRIB3", "PHLPP", 
		       "CASP9", "AKT1S1", "MDM2", "RPS6KB2" ]

        pathways["Wnt_pathway"] = [ "PPP2R5B", "PPP2R5A", "PPP2R5D", "BTRC", "WNT3A", 
    ...
    mutationTypes = [ ":y_n_somatic", ":nonsilent_somatic", ":y_del_somatic", ":y_amp_somatic" ]
	...
	for aMutType in mutationTypes:
	    ...
	    if ( 0 ):
		    ...
		    if ( not gnabLabel.startswith("B:GNAB:") ): continue
		    ...
		    if ( gnabGene in pathways[aPathway] ):
		        for iCol in range(numCol):
		            if ( dMat[iR][iCol] == 1 ):
				if ( newDataMat[kFeat][iCol] != 1 ): numON += 1
		...
		## mutated, but just a count normalized by the # of genes in the pathway
	...
	for aMutType in mutationTypes:
	    ...
	    newDataMat[kFeat] = numpy.zeros ( numCol )
	        ...
	        if ( 1 ):
	            gnabLabel = rowLabels[iR]
		    if ( not gnabLabel.startswith("B:GNAB:") ): continue
		    if ( gnabLabel.find(aMutType) < 0 ): continue
...
def combineGnabCnvr ( dataD ):
    ...
    ## next, we need to find all of the GNAB features and all of the CNVR features
	...
	if ( isGnab[iR] ):
	    ...
	    if ( gnabChrName.endswith("X") ): continue
	    if ( gnabChrName.endswith("Y") ): continue
		...
		if ( isCnvr[jR] ):
		    ...
		    if ( gnabChrName != cnvrChrName ): continue
		    ## print " comparing ... ", gnabLabel, cnvrLabel
		    cnvrStart = int ( cnvrTokens[4] )
		    if ( gnabStop < cnvrStart ): continue
		    cnvrStop  = int ( cnvrTokens[5] )
		    if ( gnabStart > cnvrStop ): continue
		    mapVec[iR] += [ jR ]
		    ## print " found match! ", gnabLabel, cnvrLabel, iR, jR, mapVec[iR]
	    ...
	    if ( curLabel.startswith("TCGA-") ):
		    ...
		    if ( sampleType == '11' ):
		        ...
		        for iRow in range(numRow):
			        ...
			        if ( curFeature.find(":chrX:") > 0 ): continue
			        if ( curFeature.find(":chrY:") > 0 ): continue
			        ## print curFeature, dMat[iRow][iCol]
		                if ( dMat[iRow][iCol]=="NA" or dMat[iRow][iCol]==NA_VALUE ):
			            dMat[iRow][iCol] = 0.
			            numCNVRset += 1
	...
	if ( isGnab[iR] ):
		...
		delFlag = 0
	        for jR in mapVec[iR]:
		    if ( dMat[iR][iCol] == 1 ): mutFlag = 1
		    if ( dMat[jR][iCol] == NA_VALUE ): continue
		    if ( dMat[jR][iCol] > maxCN ): maxCN = dMat[jR][iCol]
		    if ( dMat[jR][iCol] < minCN ): minCN = dMat[jR][iCol]
		    if ( dMat[jR][iCol] < -cnvrThreshold ): delFlag = 1
		    if ( dMat[jR][iCol] >  cnvrThreshold ): ampFlag = 1
		numYes += mutFlag
		numDel += delFlag
		numAmp += ampFlag
		...
		newFeatName = makeNewFeatureName ( curFeatName, ":y_n", ":y_del" )
		    ...
		    newDataMat += [ numpy.zeros(numCol) ]
		        ...
		        ## if we already have a 'yes' for the mutation, that's all we need ...
		        if ( dMat[iR][iCol] == 1 ): 
		            newDataMat[-1][iCol] = 1
			    continue
		        ## if not, then check for deletions ...
	                for jR in mapVec[iR]:
		            if ( dMat[jR][iCol] == NA_VALUE ): continue
		            if ( dMat[jR][iCol] < -cnvrThreshold ): newDataMat[-1][iCol] = 1
	    ...
	    if ( numAmp > 1 ):
	        numNewFeat += 1
		curFeatName = rowLabels[iR]
		newFeatName = makeNewFeatureName ( curFeatName, ":y_n", ":y_amp" )
                print "         newFeatName <%s> " % newFeatName
