...
def similarFeatureExists ( newLabel, rowLabels ):
    ...
    if ( newLabel in rowLabels ): return ( 1 )
    ...
    for tstLabel in rowLabels:
        ...
        checkThis = 0
        if ( tstLabel[:9] == newLabel[:9] ): checkThis = 1
        if ( newLabel[2:6]=="SAMP" and tstLabel[2:6]=="CLIN" ): checkThis = 1 
        if ( newLabel[2:6]=="CLIN" and tstLabel[2:6]=="SAMP" ): checkThis = 1 
...
def addIndicators4oneFeat ( aKey, rowLabels, tmpMatrix ):
    ...
    print "     uVec : ", uVec
    nCard = len(uVec)
    if ( nCard < 2 ): return ( rowLabels, tmpMatrix )
    if ( nCard > 32 ): return ( rowLabels, tmpMatrix )

    labelList = uVec
    labelList.sort()
    if ( labelList == [ '0', '1' ] ): return ( rowLabels, tmpMatrix )
...
def addIndicatorFeatures ( tmpD ):
    ...
    for iRow in range(numRow):
        ...
        if ( aFeature.find("Gistic") >= 0 ): continue
        if ( aFeature.find("batch_number") >= 0 ): continue
        if ( aFeature.find("icd_o_3_histology") >= 0 ): continue
        if ( aFeature.find("icd_o_3_site") >= 0 ): continue
        if ( aFeature.find("icd_10") >= 0 ): continue
        if ( aFeature.find("system_version") >= 0 ): continue
        if ( aFeature.find(":GNAB:") >= 0 ): continue

        if ( aFeature.startswith("N:") ):
            doNothing = 1
        else:
            ( rowLabels, tmpMatrix ) = addIndicators4oneFeat ( aFeature, rowLabels, tmpMatrix )
