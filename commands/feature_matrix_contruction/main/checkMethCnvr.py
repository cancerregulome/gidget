...
def checkMethCnvr ( dataD ):
    ...
    ## next, we need to find all of the METH features and all of the CNVR features
	...
	if ( iR%1000 == 0 ): print iR, numRow, numMatch, numNoMatch
	...
	## if this is a METH feature, find the matching CNVR ...
	    ...
	    if ( methChrName.endswith("X") ): continue
	    if ( methChrName.endswith("Y") ): continue

	    for jR in range(numRow):
		if ( mapVec[iR] >= 0 ): continue
	        if ( isCnvr[jR] ):
		    cnvrLabel = rowLabels[jR]
		    cnvrTokens = cnvrLabel.split(':')
		    cnvrChrName = cnvrTokens[3].upper()
		    if ( methChrName != cnvrChrName ): continue
		    cnvrStart = int ( cnvrTokens[4] )
		    if ( methPos < cnvrStart ): continue
		    cnvrStop  = int ( cnvrTokens[5] )
		    if ( methPos > cnvrStop ): continue
	...
	if ( iR%1000 == 0 ): 
	    ...
	    if ( numDel > 0 ): print "     deletions      ", numDel, float(sumDel)/float(numDel)
	    if ( num0   > 0 ): print "     neutral        ", num0,   float(sum0)/float(num0)
	    if ( numAmp > 0 ): print "     amplifications ", numAmp, float(sumAmp)/float(numAmp)
	    if ( numAll > 0 ): print "     all            ", numAll, float(sumAll)/float(numAll)
    ...
    if ( numDel > 0 ): print "     deletions (reset)          ", numDel, float(sumDel)/float(numDel)
    if ( num0   > 0 ): print "     neutral                    ", num0,   float(sum0)/float(num0)
    if ( numAmp > 0 ): print "     amplifications (not reset) ", numAmp, float(sumAmp)/float(numAmp)
    if ( numAll > 0 ): print "     all                        ", numAll, float(sumAll)/float(numAll)
