...
def addDerivedFeatures ( allClinDict ):
	...
	if ( keyType == "NOMINAL" ):
		...
		tmpKey = aKey.lower()
                ...
                for aLabel in labelList:
                    tmpL = str(aLabel)
                    if ( tmpL not in tmpList ): tmpList += [ tmpL ]
                labelList = tmpList
...
##		     tmpKey.find("pathologic_spread")>=0 ):
			...
			for bKey in mapDict.keys():
			    ...
			    if ( curVal.upper() not in uVec ): uVec += [ curVal.upper() ]
			    try:
			        fVal = float(curVal)
			    except:
			        isNum = 0
		...
		for aLabel in labelList:
		    ...
		    try:
		        ...
		        if ( aKey[1]==":" and aKey[6]==":" ):
                            ## if this feature name already has a prefix (eg "C:CLIN:")
                            featType = aKey[2:7]
			    i1 = aKey[7:].find(':') 
			    if ( i1 < 0 ): 
		            ...
		            newLabel = "B:" + featType + "I(" + aLabel + "|" + firstName + ")" + secondName
