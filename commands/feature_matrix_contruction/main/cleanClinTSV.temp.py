...
if ( 0 ):
    remapDict["histological_type"]["na"] = "NA"
    remapDict["histological_type"]["untreated_primary_(de_novo)_gbm"] = "de_novo"
...
remapDict["breast_carcinoma_estrogen_receptor_status"]["performed_but_not_available"] = "NA"
remapDict["breast_carcinoma_estrogen_receptor_status"]["indeterminate"] = "indeterminate"
    ...
    for aKey in keyList:
        ...
        if ( aKey2 in remapDict.keys() ):
            ...
            for kk in range(len(tmpV)):
                bKey2 = tmpV[kk].lower()
                if ( bKey2.startswith ("stage_") ): bKey2 = bKey2[6:]
                if ( bKey2.startswith ("stage ") ): bKey2 = bKey2[6:]
                try:
                    newV[kk] = remapDict[aKey2][bKey2]
                    if ( newV[kk]!="NA" and newV[kk]!="na" ):
                        if ( newV[kk].lower() != bKey2 ):
                            ## print "     remapping ... ", aKey, aKey2, kk, bKey2, remapDict[aKey2][bKey2]
                            numRemap += 1
                except:
                    if ( 0 ):
...
def computeLymphnodesFraction ( allClinDict ):
    ...
    for kk in range(numClin):
        if ( allClinDict[bKey][kk] == "NA" ):
            ...
            newV[kk] = float(allClinDict[aKey][kk]) / float(allClinDict[bKey][kk])
...
def addFollowupInfo ( allClinDict ):
    ...
    if ( haveA ):
        ...
        ## create it ...
        if ( haveB ): numClin = len(allClinDict[bKey])
        elif ( haveC ): numClin = len(allClinDict[cKey])
        else:
...
def addMissingInfo ( allClinDict ):
    ...
    barcodeList = [ "TCGA-A6-2672", "TCGA-A6-2676", "TCGA-A6-3809", "TCGA-A6-2683", "TCGA-A6-3808", "TCGA-AY-4070", "TCGA-AY-4071", "TCGA-A6-3807", "TCGA-A6-3810", "TCGA-AF-2692" ]
    tumorList   = [ "COAD",         "COAD",         "COAD",         "COAD",         "COAD",         "READ",         "READ",         "COAD",         "COAD",         "READ" ]
    tissueList  = [ "COLON",        "COLON",        "COLON",        "COLON",        "COLON",        "RECTUM",       "RECTUM",       "COLON",        "COLON",        "RECTUM" ]
    genderList  = [ "FEMALE",       "FEMALE",       "NA",           "FEMALE",       "NA",           "NA",           "NA",           "FEMALE",       "MALE",         "FEMALE" ]
...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
## derive the preferred stage tumor stage from the comparison of the reported one with the derived one
def PreferredStage( reported, computed):
    t=testTumorStage(reported,computed)
    if ( t == "AGREE" ): return(reported)
    if ( t == "Stage cannot be derived from TNM" ): return(reported)
    if ( t == "Derived stage is more specific" ):return(repStage(computed)) ## For SupTab1 use return(computed)
    if ( t == "Stage can be derived from TNM"): return(repStage(computed)) ## For SupTab1 use return(computed)
    if ( t == "Stage more specific than TNM"): return(reported)
    if ( t == "DISAGREE"): return(reported) ## assuming the reported one to be valid!
...
def repStage( substage ):
    if ( type(substage) is str ): return(substage)
    if ( type(substage) is list ): return(substage[0])
...
def testTumorStage( reported, computed ):
    ...
    if ( type(computed) is list ):
        if ( reported in computed ): return("AGREE")
    if ( type(computed) is str ):
        if ( reported==computed ): return("AGREE")
    if ( ((reported=="STAGE IVA") | (reported=="STAGE IVB")) & (computed=="STAGE IV")): return("Stage more specific than TNM")
    if ( (reported=="NA") & (computed != "NA")): return("Stage can be derived from TNM")
    if ( (reported!="NA") & (computed == "NA")): return("Stage cannot be derived from TNM")
    if ( repStage(computed).startswith(reported)): return("Derived stage is more specific")
    return("DISAGREE")
...
def getTumorStage ( T, N, M ):
    ...
    if ( M=="M1" ):  return ( "STAGE IV" ) ## Seems to be TCGA choice if IVA, IVB not specified
    ...
    if ( T=="NA"  or  N=="NA"  or  M=="NA" ): return ( "NA" )

    if ( T=="T0"  and N=="N0"  and M=="M0" ): return ( "STAGE 0" )
    if ( T=="Tis"  and N=="N0"  and M=="M0" ): return ( "STAGE 0" )

    if ( T=="T1"  and N=="N0"  and M=="M0" ): return ( "STAGE I" )
    if ( T=="T1A" and N=="N0"  and M=="M0" ): return ( "STAGE I" )
    if ( T=="T1B" and N=="N0"  and M=="M0" ): return ( "STAGE I" )
    if ( T=="T2"  and N=="N0"  and M=="M0" ): return ( "STAGE I" )
    if ( T=="T2A" and N=="N0"  and M=="M0" ): return ( "STAGE I" )
    if ( T=="T2B" and N=="N0"  and M=="M0" ): return ( "STAGE I" )

    if ( T=="T3"  and N=="N0"  and M=="M0" ): return ( "STAGE IIA" )
    if ( T=="T3A" and N=="N0"  and M=="M0" ): return ( "STAGE IIA" )
    if ( T=="T3B" and N=="N0"  and M=="M0" ): return ( "STAGE IIB" )
    if ( T=="T3C" and N=="N0"  and M=="M0" ): return ( "STAGE IIB" )
    if ( T=="T4A" and N=="N0"  and M=="M0" ): return ( "STAGE IIB" )
    if ( T=="T4B" and N=="N0"  and M=="M0" ): return ( "STAGE IIC" )
    if ( T=="T4"  and N=="N0"  and M=="M0" ): return ( ["STAGE IIB","STAGE IIC"] )

    if ( T=="T1"  and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T1A" and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T1B" and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T1"  and N=="N1A" and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T1"  and N=="N1B" and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T1"  and N=="N1C" and M=="M0" ): return ( "STAGE IIIA" )
    
    if ( T=="T1"  and N=="N2"  and M=="M0" ): return ( ["STAGE IIIA","STAGE IIIB"] ) ## CHOICE IIIA, IIIB
    if ( T=="T1B" and N=="N2"  and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T1"  and N=="N2A" and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T1"  and N=="N3"  and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T2"  and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T2A" and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T2B" and N=="N1"  and M=="M0" ): return ( "STAGE IIIA" ) 
    if ( T=="T2"  and N=="N1A" and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T2"  and N=="N1B" and M=="M0" ): return ( "STAGE IIIA" )
    if ( T=="T2"  and N=="N1C" and M=="M0" ): return ( "STAGE IIIA" )

    if ( T=="T3"  and N=="N1"  and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3A" and N=="N1"  and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3B" and N=="N1"  and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3"  and N=="N1A" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3"  and N=="N1B" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3"  and N=="N1C" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T4A" and N=="N1"  and M=="M0" ): return ( "STAGE IIIB" ) 
    if ( T=="T4A" and N=="N1A" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T4A" and N=="N1B" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T4A" and N=="N1C" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T4" and N=="N1" and M=="M0" ): return ( ["STAGE IIIB","STAGE IIIC"] )
    if ( T=="T2"  and N=="N2"  and M=="M0" ): return ( "STAGE IIIB" ) 
    if ( T=="T2A" and N=="N2"  and M=="M0" ): return ( "STAGE IIIB" ) 
    if ( T=="T2B" and N=="N2"  and M=="M0" ): return ( "STAGE IIIB" ) 
    if ( T=="T2"  and N=="N2A" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T2"  and N=="N2B" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T2"  and N=="N2C" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T2"  and N=="N3"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T2B" and N=="N3"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T3"  and N=="N2"  and M=="M0" ): return ( ["STAGE IIIB","STAGE IIIC"] ) ## CHOICE IIIB, IIIC
    if ( T=="T3"  and N=="N2A" and M=="M0" ): return ( "STAGE IIIB" )
    if ( T=="T3"  and N=="N2C" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T1"  and N=="N2B" and M=="M0" ): return ( "STAGE IIIB" )

    if ( T=="T3"  and N=="N2B" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T3"  and N=="N3"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4"  and N=="N2"  and M=="M0" ): return ( "STAGE IIIC" ) 
    if ( T=="T4A" and N=="N2"  and M=="M0" ): return ( "STAGE IIIC" ) 
    if ( T=="T4A" and N=="N2A" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4A" and N=="N2B" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4A" and N=="N2C" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4"  and N=="N3"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4"  and N=="N3A" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4A" and N=="N3"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N1"  and M=="M0" ): return ( "STAGE IIIC" ) 
    if ( T=="T4B" and N=="N2"  and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N1A" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N1B" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N1C" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N2C" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N2A" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N2B" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N2B" and M=="M0" ): return ( "STAGE IIIC" )
    if ( T=="T4B" and N=="N3A" and M=="M0" ): return ( "STAGE IIIC" )

    ## We reach this point if all values are non-NA, but combination is not in AJCC tumor table
...
def checkTumorStage ( allClinDict ):
    ...
    reqKeyList = [ "bcr_patient_barcode", "tumor_stage", "primary_tumor_pathologic_spread", "lymphnode_pathologic_spread", "distant_metastasis_pathologic_spread" ]
    numNotFound = 0
    ...
    for ii in range(numClin):
        ...
        ## print " checking tumor stage for <%s> <%s> <%s> <%s> <%s> " % ( aCode, curTumorStage, curN, curM, curT )
        ...
        if ( 0 ):
            ...
            ## get AJCC-derived tumor stage, compare to DCC value, and decide which to use
            ...
            print aCode.upper() + ', TNM:' + curT.upper() + ' ' + curN.upper() + ' ' + curM.upper() + ', DCC Stage:' \
                                ...
                                + testTumorStage(curTumorStage,ajccStage) + ', Will use: ' + newStage
...
def removeParens ( oneKey ):
    ...
    for aLabel in oneKey:
        ...
        if ( aLabel not in origUniqList ): origUniqList += [ aLabel ]
        ...
        if ( aLabel.find("(") >= 0 ):
            ...
            if ( bLabel.startswith("_") ): bLabel = bLabel[1:]
            if ( bLabel.endswith("_") ): bLabel = bLabel[:-1]

            newKey += [ bLabel ]
            if ( bLabel not in newUniqList ): newUniqList += [ bLabel ]

        else:

            newKey += [ aLabel ]
            if ( aLabel not in newUniqList ): newUniqList += [ aLabel ]
...
def removeSpecialChars ( oneKey ):
    ...
    if ( numInt>0 and numNot>0 ):
        ...
        ## for now, we are just checking for 'grade' strings that are sometimes 'grade_3' and sometimes just '3'
        if ( aString.startswith("grade_") ):
            for ii in range(len(oneKey)):
                aLabel = str ( oneKey[ii] )
                if ( aLabel.upper() == "NA" ): continue
...
def getCommonPrefix ( aLabel, bLabel ):
    ...
    while ( aLabel[nn].lower() == bLabel[nn].lower() ):
        nn += 1
        if ( nn >= len(aLabel) ): return ( aLabel[:nn].lower() )
        if ( nn >= len(bLabel) ): return ( aLabel[:nn].lower() )
...
def getCommonSuffix ( aLabel, bLabel ):
    ...
    while ( aLabel[nn].lower() == bLabel[nn].lower() ):
        nn -= 1
        if ( -nn > len(aLabel) ): return ( aLabel )
        if ( -nn > len(bLabel) ): return ( bLabel )
...
def removeCommonPrefix ( oneKey, labelList ):
    ...
    for ii in range(nLabel):
        for jj in range(ii+1,nLabel):
            ...
            if ( commonPrefix == labelList[ii] ): continue
            if ( commonPrefix == labelList[jj] ): continue
    ...
    if ( madeChange > 0 ): print " after removeCommonPrefix : ", madeChange, labelList
...
def removeCommonSuffix ( oneKey, labelList ):
    ...
    for ii in range(nLabel):
        for jj in range(ii+1,nLabel):
            ...
            ## we cannot really use this ...
            if ( commonSuffix == labelList[ii] ): continue
            if ( commonSuffix == labelList[jj] ): continue
    ...
    if ( madeChange > 0 ): print " after removeCommonSuffix : ", madeChange, labelList
...
def abbrevCategStrings ( allClinDict ):
    ...
    for aKey in keyList:
        ...
        if ( aKey == "bcr_patient_barcode" ): continue
        ...
        if ( keyType == "NOMINAL" ):
            ...
            if ( maxLen > 10 ):
                ...
                ## first try at making the labels a bit shorter by removing parenthetical elements ...
                ...
                if ( 0 ):
                    ...
                    if ( maxLen > 10 ):
                        ( allClinDict[aKey], labelList ) = removeCommonPrefix ( allClinDict[aKey], labelList )
                        ( allClinDict[aKey], labelList ) = removeCommonSuffix ( allClinDict[aKey], labelList )
        ...
        if ( keyType == "NOMINAL" ):
            if ( nCard > 2  and  nCard < 15 ):
                ...
                if ( tmpKey.find("stage")>=0  or  tmpKey.find("grade")>=0  or
                    ...
                    if ( prefixBits[4] ): prefixBits[5] = 0

                    ## print prefixBits
                    usePrefix = ""
                    if ( sum(prefixBits) == 1 ):
                        for iP in range(nPrefix):
                            if ( prefixBits[iP] ): usePrefix = prefixList[iP]
                    ...
                    if ( aKey[1] == ":" ):
                        ...
                        newLabel = "N:" + tokenList[1] + ":" + tokenList[2] + "_derived" 
        ...
        if ( keyType == "NOMINAL" ):
            if ( nCard > 2  and  nCard < 15 ):
                ...
                if ( tmpKey.find("stage")>=0  or  tmpKey.find("grade")>=0  or
                    ...
                    ## if there is no mapping file, then we won't be making any new features ...
                    if ( len(newNames) == 0 ): continue
                    ...
                    for ithName in range(len(newNames)):
                        ...
                        for bKey in mapDict.keys():
                            ...
                            if ( curVal not in uVec ): uVec += [ curVal ]
                            try:
                                fVal = float(curVal)
                            except:
                                isNum = 0
                        ...
                        if ( len(uVec) == 1 ):
                            ...
                            ## if the mapping produces a binary feature, then over-ride the numerical feature
                            if ( isNum ):
                        ...
                        for kk in range(numClin):
                            if ( curV[kk].upper() == "NA" ):
                                ...
                                try:
                                     tmpV[kk] = mapDict[curV[kk].upper()][ithName]
                                except:
                                    print " failed to map ??? "
                    ...
                    try:
                        if ( aKey[1]==":" and aKey[6]==":" ):
                            ...
                            if ( i2>0 and i2<len(aKey) ):
                                newLabel = "B:" + aKey[2:7] + "I(" + aLabel + "|" + aKey[7:i1] + ")" + aKey[i2:]
                            else:
                                newLabel = "B:" + aKey[2:7] + "I(" + aLabel + "|" + aKey[7:i1] + ")" + "::::"
                            if ( newLabel.find("|):") > 0 ):
                            ...
                            newLabel = "B:" + typeString + ":" + "I(" + aLabel + "|" + aKey + ")"
                    except:
                        print " (b) BAILING !!! "
                    ...
                    for bk in range(ak+1,len(labelList)):
                        ...
                        try:
                            if ( aKey[1]==":" and aKey[6]==":" ):
                                ...
                                if ( i2>0 and i2<len(aKey) ):
                                    newLabel = "B:" + aKey[2:7] + "I(" + aLabel + "," + bLabel + "|" + aKey[7:i1] + ")" + aKey[i2:]
                                else:
                                    newLabel = "B:" + aKey[2:7] + "I(" + aLabel + "," + bLabel + "|" + aKey[7:i1] + ")" + "::::"
                                ...
                                if ( typeString == "UNK" ):
                                    print " defaulting to type SAMP for this feature : <%s> " % ( aKey )
                                    typeString = "SAMP"
                                newLabel = "B:" + typeString + ":" + "I(" + aLabel + "," + bLabel + "|" + aKey + ")" + "::::"
...
if __name__ == "__main__":
    ...
    ## automatically generate indicator features for remaining categorical features
    ...
    if ( 1 ):
        ...
        tsvIO.writeTSV_clinicalFlipNumeric ( allClinDict, bestKeyOrder, outName )
