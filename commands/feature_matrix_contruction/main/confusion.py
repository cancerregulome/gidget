...
def printConfusionMatrix ( aLine, bLine ):
    ...
    aTokens = aTokens[1:]
    for aTok in aTokens:
        if ( aTok not in aVals ): aVals += [ str(aTok) ]

    bTokens = bLine.split('\t')
    bName = bTokens[0]
    bTokens = bTokens[1:]
    for bTok in bTokens:
        if ( bTok not in bVals ): bVals += [ str(bTok) ]
    ...
    for aTok in aVals:
        ...
        for bTok in bVals:
            ...
            for ii in range(len(aTokens)):
                if ( aTokens[ii]==aTok and bTokens[ii]==bTok ): numAB += 1
            outLine += " %4d " % numAB
        print outLine
...
if __name__ == "__main__":
    ...
    for aLine in fh:
        ...
        if ( aLine.startswith("N:") ): continue
        ## if ( aLine.find(":I(") >= 0 ): continue
        aLine = aLine.strip()
        if ( aLine.find(feat1) >= 0 ): list1 += [ aLine ]
        if ( aLine.find(feat2) >= 0 ): list2 += [ aLine ]
    fh.close()
