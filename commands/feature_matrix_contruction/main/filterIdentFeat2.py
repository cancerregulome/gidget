...
def chooseMostSpecific ( bestFeatureList ):
    ...
    if ( bestFeatureList[0].find("y_amp_") >= 0 ): return ( bestFeatureList[1] )
    if ( bestFeatureList[1].find("y_amp_") >= 0 ): return ( bestFeatureList[0] )
    if ( bestFeatureList[0].find("y_del_") >= 0 ): return ( bestFeatureList[1] )
    if ( bestFeatureList[1].find("y_del_") >= 0 ): return ( bestFeatureList[0] )
	...
	if ( geneName not in geneList ):
	    ...
	    geneCounts[jj] += 1
    if ( len(geneList) > 1 ): return ( '' )

    ## these are the mutation strings in order of decreasing specificity
    infixList = [ ":ddG_", ":iarc_", ":tcga_", ":code_potential", ":mni_", ":mnf_", ":dom_", ":dna_bin", ":DNA_interface", ":missense", ":nonsilent", ":y_n" ]
    NSorFS = ":ns_or_fs"
    ORstring = "_OR"
    ...
    if ( len(bestFeatureList) == 2 ):
        if ( bestFeatureList[0].endswith("_OR") and bestFeatureList[1].endswith("_germline") ): return ( bestFeatureList[1] )
        if ( bestFeatureList[1].endswith("_OR") and bestFeatureList[0].endswith("_germline") ): return ( bestFeatureList[0] )
        if ( bestFeatureList[0].endswith("_OR") and bestFeatureList[1].endswith("_somatic") ): return ( bestFeatureList[1] )
        if ( bestFeatureList[1].endswith("_OR") and bestFeatureList[0].endswith("_somatic") ): return ( bestFeatureList[0] )
