    ...
    if ( 1 ):
        ...
        for ii in range(max(len(aTokens),len(bTokens))):
            ...
            if ( ii < len(aTokens) ):
                ...
                if ( aTokens[ii] == "amino_acid_change_WU" ): iPChange = ii
                if ( aTokens[ii] == "amino_acid_change" ): iPChange = ii
                if ( aTokens[ii] == "AAChange" ): iPChange = ii
...
def getMutDictFromMaf ( mafFilename, geneList ):
    ...
    for aLine in fh:
        ...
        if ( pChange1.startswith("p.") ): pChange1 = pChange1[2:]
