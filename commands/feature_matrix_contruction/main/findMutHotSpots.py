...
def getColumnIndices ( mafFilename ):
    ...
    global iHugo, iBarcode, iVarClass, iVarType, iPChange, iAnnot
    ...
    if ( 1 ):
        ...
        for ii in range(max(len(aTokens),len(bTokens))):
            ...
            if ( ii < len(aTokens) ):
                ...
                if ( aTokens[ii] == "Variant_Classification" ): iVarClass = ii
                if ( aTokens[ii] == "Variant_Type" ): iVarType = ii
                if ( aTokens[ii] == "Protein_Change" ): iPChange = ii
                if ( aTokens[ii] == "Mutation Annotation (Others)" ): iAnnot = ii

        print " "
        print " "
        fh.close()

    if ( iHugo<0 or iBarcode<0 or iVarClass<0 or iVarType<0 or iPChange<0 or iAnnot<0 ):
        print " ERROR in getColumnIndices ... failed to find one or more required columns "
        print iHugo, iBarcode, iVarClass, iVarType, iPChange, iAnnot
        sys.exit(-1)
...
def getMutDictFromMaf ( mafFilename, geneList ):
    ...
    for aLine in fh:
        ...
        pChange2 = getProteinChangefromAnnot ( tokenList[iAnnot] )

        print hugoSymbol, barcode, varClass, varType, pChange1, pChange2
