...
if __name__ == "__main__":
    ...
    for iRow in range(numRow):
	    ...
	    try:
	        ...
	        if ( minVal == NA_VALUE ): minVal = curVal
	        if ( maxVal == NA_VALUE ): maxVal = curVal
	...
	if ( ii < 21 ):
	    ## print " --> iRow=%d does not have any (or enough) data (%d) non-NA samples ??? " % ( iRow, ii )
	    numSkip += 1
	    continue
	tmpV = tmpV[:ii]
	numVec[iRow] = ii
	...
	a2dHist = plotMatrix.makePlotMatrix ( xMin, xMax, xDel, yMin, yMax, yDel )
	for ii in range(len(medVec)):
	    a2dHist = plotMatrix.addSampleToPlotMatrix ( a2dHist, medVec[ii], madVec[ii], 1 )
    ...
    for iRow in range(numRow):
	if ( strictStat ):
            if ( method == "MAD" ): tmpVec[iRow] = madVec[iRow]
            if ( method == "IQR" ): tmpVec[iRow] = iqrVec[iRow]
            if ( method == "IDR" ): tmpVec[iRow] = idrVec[iRow]
            if ( method == "ITR" ): tmpVec[iRow] = tweVec[iRow]
	    if ( method == "NZC" ): tmpVec[iRow] = nzcVec[iRow]
	else:
	    ## doing it this way also penalizes the lowly-expressed genes ...
	    ## with the second term knocked down a bit so that it doesn't dominated ...
            if ( method == "MAD" ): tmpVec[iRow] = madVec[iRow] + medVec[iRow]/6.
            if ( method == "IQR" ): tmpVec[iRow] = iqrVec[iRow] + qtrVec[iRow]/2.
            if ( method == "IDR" ): tmpVec[iRow] = idrVec[iRow] + decVec[iRow]/1.
            if ( method == "ITR" ): tmpVec[iRow] = itrVec[iRow] + tweVec[iRow]/1.
	    if ( method == "NZC" ): tmpVec[iRow] = nzcVec[iRow]
    ...
    if ( topFrac > 0. ):
        ...
        if ( iFrac > 1 ): iFrac -= 1
	if ( iFrac < 0 ): iFrac = 0
	if ( iFrac > (len(tmpVec)-1) ): iFrac = len(tmpVec) - 1
	try:
	...
	if ( topCount > 0 ):
	    ...
	    while ( tmpVec[iFrac] < abs(topCount) ): iFrac += 1
	...
	if ( threshVal < 3 ): threshVal = 3
	print " --> forcing threshVal to %d " % threshVal
	jFrac = 0
	print tmpVec[:5]
	print tmpVec[-5:]

    ## now, walk forward as long as the threshold does not change, and reset the position
	...
	## we will not filter out categorical features based on "high variability"
	if ( rowLabels[iRow].startswith("C:") ): continue

	## and we won't filter out binary features unless the filter type is NZC
	if ( method != "NZC" ):
	    if ( rowLabels[iRow].startswith("B:") ): continue
	...
	## and here is where we actually decide whether or not to remove this feature
