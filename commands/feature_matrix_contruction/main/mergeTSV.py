	...
	## we will just assume that we keep it ...
		...
		doNothing = 1
	        ...
	        aCode = miscTCGA.fixTCGAbarcode ( aCode )
                ( site, patient, sample, vial, portion, analyte, plate, center ) = miscTCGA.parseTCGAbarcode ( aCode )
	        try:
	            iSample = int(sample)
	        except:
	            iSample = -1
...
def removeDuplicateSamples ( dataD, barcodeLen, firstLast=0 ):
    ...
    for aKey in allKeys:
	    ...
	    ## now decide which one to keep ... we prefer "R" over "T" apparently ...
	    keepCode = "NA"
...
if __name__ == "__main__":
	...
	if ( 0 ):
	    ...
	    if ( skipRowList != [] ): testD = tsvIO.filter_dataMatrix ( testD, skipRowList, [] )
            tsvIO.lookAtDataD ( testD )

	    skipColList = tsvIO.getSkipList ( colMaxNAfrac, testD, 'col' )
            if ( skipColList != [] ): testD = tsvIO.filter_dataMatrix ( testD, [], skipColList )
            tsvIO.lookAtDataD ( testD )
    ...
    ## now we build up a union of the lists of features (row) and samples (columns)
		...
		if ( outMatrix[iu][ju]!=NA_VALUE and outMatrix[iu][ju]!="NA" ):
		    if ( inputData[iD]['dataMatrix'][ii][jj]!=NA_VALUE and inputData[iD]['dataMatrix'][ii][jj]!="NA" ):
		        if ( outMatrix[iu][ju] != inputData[iD]['dataMatrix'][ii][jj] ):
		            ## print "     --> WARNING: not replacing previously set value ... ", iu, ju, outMatrix[iu][ju], inputData[iD]['dataMatrix'][ii][jj], curRowLabels[ii], curColLabels[jj]
			    doNothing = 1
		    continue
		else:
