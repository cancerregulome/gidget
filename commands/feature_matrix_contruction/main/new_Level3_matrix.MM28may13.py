...
def readOneDataFile ( fName, geneList, zPlat, metaData ):
	...
	if ( makeGeneList ):
		...
		try:
		    ## print metaData[aCG]
		    ## metaData[probeID] = ( probeID, geneName, chrName, chrPos, chrStrand )
...
def resegmentCNdata ( sampleList, segMatrix, dataMatrix, 
    ...
    chr2maxcoord = {}
    chrNames = []
    print datetime.now(), 'begin transforming data'
	...
	##	and	(chr,start,stop) are extracted from segMatrix[iS][jS]
        maxchrcoord = 0
        print 'transforming data for chr%s' % (chr)
	for jS in range(numSamples):
		...
		data[chr][jS][start] = (stop, value)
                stopcoord = int(int(stop)/steplength)        ## floor
                maxchrcoord = max ( maxchrcoord, stopcoord )
        chr2maxcoord[chr] = maxchrcoord
	## end of loop over numSamples
    print datetime.now(), 'end transforming data'
    print datetime.now(), 'calling resegment._resegmentChromosomes(): %i %i %.2f' % (len(sampleList), steplength, cutFrac)
    segList, sampleList, newMatrix = resegment._resegmentChromosomes(chrNames, data, chr2maxcoord, sampleList, steplength, cutFrac)
    print datetime.now(), 'end resegmenting data'
