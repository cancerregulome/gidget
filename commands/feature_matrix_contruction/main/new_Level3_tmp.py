...
def getSDRFinfo ( sdrfFilename ):
	...
	if ( len(aTokens) < numTokens ):
	    ...
	    ## print " in getSDRFinfo:   line # %4d    zPlat=<%s>    numTokens=%d " % ( lineNum, zPlat, numTokens )
		...
		if ( numTokens == 26 ):
		    ## this should be correct BOTH for UNC BCGSC Illumina GA RNAseq data ...
		    iLevel3 = 24
		    iBarcode = 1
		    iFilename = 21
		    iArchive = 25
		    ...
		    if ( aTokens[iOther].find("gene_expression") < 0 ): 
		        ## print " IlluminaGA_RNASeq : not including <%s> " % aTokens[iFilename]
		        continue
		...
		if ( aTokens[2].startswith("unc.edu") or aTokens[3].startswith("unc.edu") ):
			...
			iOther = 19
		        ...
		        iOther = 20
		    if ( aTokens[iOther].find("gene_expression") < 0 ): 
		        ## print " IlluminaHiSeq_RNASeq : not including <%s> " % aTokens[iFilename]
		        continue
		...
		if ( aTokens[iOther] != "RSEM_genes_normalized" ):
		    ## print " IlluminaHiSeq_RNASeqV2 : not including <%s> " % aTokens[iFilename]
		    continue
		...
		if ( aTokens[iOther] != "RSEM_genes_normalized" ):
		    ## print " IlluminaGA_RNASeqV2 : not including <%s> " % aTokens[iFilename]
		    continue
		...
		if ( aTokens[iOther].find("Expression-miRNA Isoform") >= 0 ): 
		    ## print " IlluminaGA_miRNASeq : not including <%s> " % aTokens[iFilename]
		    continue
		...
		if ( len(aTokens) == 26 ):
		    ...
		    iOther = 22
		if ( aTokens[iOther].find("Expression-miRNA Isoform") >= 0 ): 
		    ## print " IlluminaHiSeq_miRNASeq : not including <%s> " % aTokens[iFilename]
		    continue
		...
		if ( sdrfFilename.find("broad.mit.edu") > 0 ):
		    ...
		    ## this platform is a bit of a problem right now because of the barcode to UUID transition ...
		...
		if ( not barcode.startswith("TCGA-") ):
		    ## print " MDA_RPPA_Core : not including <%s> " % aTokens[iFilename]
		    continue
	    ...
	    ## verify that the correct token says "level 3" (or "level 1" for microsat_i data)
	    try:
		...
		if ( len(barcode) > 28 ): barcode = barcode[:28]
	    ...
	    ## if this is an Agilent platform, then we only want this if it is the cy5 channel ...
    ...
    ## now we want to build a list of archives and a list of files from the sdrfDict ...
    archiveList = []
    fileList = []
...
def cleanString ( aString ):
    ...
    if ( bString.startswith("stage_") ): bString = bString[6:]
    if ( bString.startswith("grade_") ): bString = bString[6:]
...
def makeFeatureName ( dType, fType, fName, chr='', start=-1, stop=-1, strand='', xName='' ):
	...
	if ( len(RPPAdict) == 0 ):
	    ...
	    fh = file ( "/titan/cancerregulome11/TCGA/repositories/rppa/MDA_antibody_annotation.txt" )
	    for aLine in fh:
		aLine = aLine.strip()
		aLine = aLine.split('\t')
		if ( len(aLine) == 2 ):
	    ...
	    fh.close()
	## move the 'gene name' to the 'extra stuff', and then get the gene name from RPPAdict
	xName = fName
    ...
    if ( chr != '' ): tmpName += "chr" + chr 
    tmpName += ":"

    ## add (start) position
    if ( start >= 0 ): tmpName += "%d" % start
    tmpName += ":"

    ## add stop position
    if ( stop >= 0 ): tmpName += "%d" % stop
    tmpName += ":"

    if ( strand != '' ): tmpName += "%s" % strand
    tmpName += ":"

    if ( xName != '' ): tmpName += "%s" % xName
...
def readOneDataFile ( fName, geneList, zPlat, metaData ):
    ...
    ##			gene quantification data has 4 columns : gene / counts / length_norm / RPKM
	...
	if ( makeGeneList ):
	    ...
	    ## make a unique identifier using the chr and genomic coordinate and CpG id
		...
		featureName = makeFeatureName ( "N", "METH", geneName, aChr, iPos, -1, '', aCG )
		...
		try:
		    ...
		    featureName = makeFeatureName ( "N", "METH", geneName, aChr, iPos, -1, '', lastToken )
		    geneList += [ featureName ]
		    print featureName, len(geneList)
		except:
		    ## print " (x) this probe is not on the 27K platform ", cTokens
		    continue
	    ...
	    ## numeric id is added on as the 'extra' information at the end of the feature name
		...
		if ( tmpTokens[0] != "?" ):
		    ...
		    ## also, the BCGSC gene names can look like this: AMY1A|276|1of3_calculated
		...
		## print " (b) "
		featureName = makeFeatureName ( dType, fType, geneName, '', -1, -1, '', xName )
		geneList += [ featureName ]
		...
		try:
			...
			sys.exit(-1)
		## print " (c) "
	        segName = makeFeatureName ( dType, fType, "", aChr, iStart, iStop )
		geneList += [ segName ]
	...
	## otherwise, make sure that the gene names still match what we already had ...
	else:
	    ...
	    if ( zPlat == "HumanMethylation27_OBSOLETE_CODE" ):
		    ...
		    tmpName = makeFeatureName ( dType, fType, geneName, aChr, iPos, -1, '', aCG )
		except:
		    ...
		    try:
		        ...
		        iPos = int ( metaData[aCG][3] )
                        lastToken = metaData[aCG][5]
		        tmpName = makeFeatureName ( dType, fType, geneName, aChr, iPos, -1, '', lastToken )
		    except:
		...
		## print " (f) "
		tmpName = makeFeatureName ( dType, fType, geneName, '', -1, -1, '', xName )
...
def lookAtSegs ( segMatrix ):
	...
	for jS in range(numSamples):
	    ...
	    for iS in range(maxNumSeg):
	        if ( segMatrix[iS][jS] == '' ): continue
	        if ( not segMatrix[iS][jS].startswith(pref) ): continue
...
def prettyPrint ( aVec, skipFlag=1 ):
    ...
    for ii in range(1,len(aVec)-1):
	    ...
	    lastVal = aVec[ii]
	if ( not skipFlag ): lastVal = (NA_VALUE + NA_VALUE)
...
def computeFirstDiffVec ( cnvsum ):
	...
	## only if we have two valid values do we compute the difference
	    ...
	    if ( minDiff > diffvec[kk] ): minDiff = diffvec[kk]
	    if ( maxDiff < diffvec[kk] ): maxDiff = diffvec[kk]
...
def removeZerosNAs ( diffvec ):
    ...
    n2 = n1 - n0
    ## print " starting with %d values, removing %d values, left with %d values " % ( n1, n0, n2 )
    if ( n2 < 10 ):
        print " ERROR ??? "
    ...
    ## print " returning diffvec_nz ... ", len(diffvec_nz), nn, min(diffvec_nz), max(diffvec_nz)
    return ( diffvec_nz )
...
def copyInto ( cnvvec, start, stopPlus1, value ):
    ...
    if ( value == NA_VALUE ): return ( cnvvec )
	...
	for ii in range(len(cnvvec)):
	    if ( abs(cnvvec[ii]-value)<0.001 ): numSet += 1
...
def resegmentCNdata ( sampleList, segMatrix, dataMatrix, 
	...
	##          barcode                  'CNVRorig'  chr     start       stop     value,   start      stop     value, ...
	...
	## this vector will contain the sum, across all segments, of the binned CN values
	...
	## and for each sample, loop over all of the segments, sorted by start position

	for jS in range(numSamples):
	    barcode = barcodeList[jS]

	    ## print " B ", barcode, " allocating vector of zeros of length ", maxchrcoord
	    cnvvec = initializeVec ( maxchrcoord, NA_VALUE )

            ## if we have no segments, there is nothing to be done ...
            if ( len(startKeys) == 0 ): continue
		...
		for stop in data[barcode]['CNVorig'][chr][start].keys():
		    ...
		    ## print " "
		    ## print " calling copyInto ... ", startcoord, stopcoord+1, data[barcode]['CNVorig'][chr][start][stop]
		    cnvvec = copyInto ( cnvvec, startcoord, (stopcoord+1), data[barcode]['CNVorig'][chr][start][stop] )
	...
	## bins for this chromosome, and the second is the # of bins with non-zero slope)
	...
	while ( diffcoord < ndiff ):
	    ...
	    ## print " diffcoord=%6d    diff=%.1f  (%.1f) " % ( diffcoord, diff, lastDiff )

	    ## if the current diff is NA, but the previous was not, then force a break-point
	    if ( diff == abs(NA_VALUE) ):
	        if ( lastDiff < abs(NA_VALUE) ): makeBreakPoint = 1

	    ## OR if the current diff exceeds the threshold, then force a break-point
		...
		## print "             difference is significant", diff, startcoord, stopcoord, startpos, stoppos

		## print " F5  looping over barcodes ... ", barcodeList[:5], " ... ", barcodeList[-5:]

		for jS in range(numSamples):
		    barcode = barcodeList[jS]
	            if ('CNVtemp' in data[barcode].keys()):
		        ## print " getLocalMean ... ", barcode, chr, startcoord, stopcoord+1
			## print data[barcode]['CNVtemp'][chr][startcoord:(stopcoord+1)]
			( mu, numMu ) = getLocalMean ( data[barcode]['CNVtemp'][chr][startcoord:(stopcoord+1)] )
			## print " --> mu=%f  numMu=%d " % ( mu, numMu )
	                data[barcode]['CNV'][chr][startpos][stoppos] = mu

			if ( jS == 0 ):
	        	    segName = makeFeatureName ( "N", "CNVR", "", chr, startpos, stoppos )
			    segList += [ segName ]
	    ...
	    ## else:
	    ##	print " diffcoord=%2d  startcoord=%2d  stopcoord=%2d  startpos=%8d stoppos=%8d " % ( diffcoord, startcoord, stopcoord, startpos, stoppos )

	    ## if we did not make a break-point but the diff was NA, then reset the lastcoord ...
    ...
    dataMatrix = [0] * numSeg
    ## print " looping over %d segments and %d samples ... flipping matrix ... " % ( numSeg, numSamples )
    numNA = 0
...
if __name__ == "__main__":
	...
	if ( platformID.find("microsat_i") > 0 ):
	    topDir = "/titan/cancerregulome11/TCGA/repositories/" + snapshotName + "/secure/tumor/" + zCancer + "/cgcc/" + platformID
	...
	if ( len(mergeSdrfDict)==0 ):
	    ...
	    ## TCGA-G4-6309-01A-21D-1834-01 ('broad.mit.edu_COAD.Genome_Wide_SNP_6.Level_3.138.2001.0', 'BAIZE_p_TCGA_b138_SNP_N_GenomeWideSNP_6_G10_808888.hg19.seg.txt', 'Copy Number Results-SNP')
	...
	dMatch = "Level_3"
	if ( zPlat == "microsat_i" ): dMatch = "Level_1"
	...
	for dName in d1.dirs():
		...
		if ( mergeArchiveList[0] != 'unknown' ):
		    if ( archiveName not in mergeArchiveList ): continue
		...
		for fName in d2.files():
			...
			## -----------------------------------------------------------------
			## these platforms have just one sample's worth of data per file
			if ( (zPlat == "HT_HG-U133A") or (zPlat == "AgilentG4502A_07_1")   or (zPlat == "AgilentG4502A_07_2") 
					  ...
					  or (zPlat == "microsat_i") ):
			    ( barcode, info ) = getSampleID ( mergeSdrfDict, fileName )
			    print barcode, info
			    sampleList += [ barcode ]
			    ( geneList, dataVec ) = readOneDataFile ( fName, geneList, zPlat, metaData )
			    ## print geneList[:10]
			    ## print dataVec[:10]
				...
				## number of genes but rather a variable number of segmentations ...
				if ( zPlat == "Genome_Wide_SNP_6" ):
				    ...
				    ##  the SDRF file???)
				    numGenes = 25000
				    numGenes = 60000 ## trying to handle LAML ??? 22feb13
				else:
				...
				if ( zPlat == "Genome_Wide_SNP_6" ):
				    ( geneList, dataVec ) = mergeSegments ( geneList, dataVec )
				else:
			    ...
			    if ( not logFlag ):
				## open a log file to which we will write the specs for all of the files used ...
				fhLog = openDataFreezeLogFile ( zCancer, outSuffix, platformStrings )
				logFlag = 1
			    ...
			    ## if this is the GWS6 snp chip, we need to copy the segment positions too ...
			    if ( zPlat == "Genome_Wide_SNP_6" ):
				for iG in range(len(dataVec)):
	...
	if ( 1 ):
	    ## this is better for producing variable-length, hopefully uncorrelated, features
	    steplength = 1000
	    cutFrac = 0.01
	...
	if ( 0 ):
	    ...
	    steplength = 100000
	    cutFrac = 1.00
	( segList, sampleList, dataMatrix ) = resegmentCNdata ( sampleList, segMatrix, dataMatrix, steplength, cutFrac )
        ...
        try:
    	    ...
    	    newFeatureName = "C:SAMP:" + dataTypeDict[zPlat][1].lower() + "Platform"
