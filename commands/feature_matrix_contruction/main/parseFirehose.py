...
def getDate ( dName ):
    ...
    dateString = dName[-10:]
    iDate = int(dateString[0:4])*10000 + int(dateString[5:7])*100 + int(dateString[8:10])
    return ( iDate )
...
def parseBestClusFiles ( lastDir, outDir ):
    ...
    for d2Name in d2.dirs():
        ...
        if ( d2Name.find("Level_4") > 0 ):
		    ...
		    ## now let's look at all of the clusters and see how they rank ...
		    if ( iMin != 1 ):
			...
			for aKey in dataDict.keys():
			    ...
			    ## print " old cluster #%d --> new cluster #%d " % ( iClus, jClus )
		    ...
		    fhOut.write ( "bcr_patient_barcode\tC:SAMP:%s_%s_%d::::\tN:SAMP:%s_%s_%d_sv::::\n" % \
...
def parseMutSig_CountsRatesFile ( inName, outName ):
    ...
    goodData = 1
    if ( numTokens == 8 ):
        if ( hdrTokens[1] != "name" ): goodData = 0
        if ( hdrTokens[6] != "rate_sil" ): goodData = 0
        if ( hdrTokens[7] != "rate_non" ): goodData = 0
    elif ( numTokens == 10 ):
        if ( hdrTokens[ 2] != "name" ): goodData = 0
        if ( hdrTokens[ 7] != "rate_dbsnp" ): goodData = 0
        if ( hdrTokens[ 8] != "rate_sil" ): goodData = 0
        if ( hdrTokens[ 9] != "rate_non" ): goodData = 0
    elif ( numTokens == 11 ):
        if ( hdrTokens[ 2] != "name" ): goodData = 0
        if ( hdrTokens[ 8] != "rate_dbsnp" ): goodData = 0
        if ( hdrTokens[ 9] != "rate_sil" ): goodData = 0
        if ( hdrTokens[10] != "rate_non" ): goodData = 0
    elif ( numTokens == 12 ):
	if ( hdrTokens[ 2] != "name" ): goodData = 0
        if ( hdrTokens[ 9] != "rate_dbsnp" ): goodData = 0
        if ( hdrTokens[10] != "rate_sil" ): goodData = 0
        if ( hdrTokens[11] != "rate_non" ): goodData = 0
    elif ( numTokens == 13 ):
        if ( hdrTokens[ 0] != "name" ): goodData = 0
        if ( hdrTokens[11] != "rate_sil" ): goodData = 0
        if ( hdrTokens[12] != "rate_non" ): goodData = 0
    ...
    print "     writing out <%s> " % outName
    if ( numTokens == 8 ):
        fhOut.write ( "bcr_patient_barcode\tN:SAMP:MutSig_rateSil_perMb:::::\tN:SAMP:MutSig_rateNon_perMb:::::\tN:SAMP:MutSig_rateTot_perMb:::::\tB:SAMP:MutSig_HiMut_bit:::::\n" )
    elif ( numTokens == 10 ):
        fhOut.write ( "bcr_patient_barcode\tN:SAMP:MutSig_rateDbSnp_perMb:::::\tN:SAMP:MutSig_rateSil_perMb:::::\tN:SAMP:MutSig_rateNon_perMb:::::\tN:SAMP:MutSig_rateTot_perMb:::::\tB:SAMP:MutSig_HiMut_bit:::::\n" )
    elif ( numTokens == 11 ):
        fhOut.write ( "bcr_patient_barcode\tN:SAMP:MutSig_rateDbSnp_perMb:::::\tN:SAMP:MutSig_rateSil_perMb:::::\tN:SAMP:MutSig_rateNon_perMb:::::\tN:SAMP:MutSig_rateTot_perMb:::::\tB:SAMP:MutSig_HiMut_bit:::::\n" )
    elif ( numTokens == 12 ):
        fhOut.write ( "bcr_patient_barcode\tN:SAMP:MutSig_rateDbSnp_perMb:::::\tN:SAMP:MutSig_rateSil_perMb:::::\tN:SAMP:MutSig_rateNon_perMb:::::\tN:SAMP:MutSig_rateTot_perMb:::::\tB:SAMP:MutSig_HiMut_bit:::::\n" )
	...
	if ( len(tokenList) != numTokens ):
	    ...
	    rateNon_perMb   = rateNon   * 1000000.
            ...
            ## 31oct13 : the "total" now includes the dbSNP rate as well (if it was available)
...
def parseMutSig_SampleSigGeneFile ( inName, outName ):
	...
	if ( len(tokenList) != numTokens ):
		...
		if ( minVal > mutCount ): minVal = mutCount
		if ( maxVal < mutCount ): maxVal = mutCount
		sumMut += (mutCount>0)
...
def parseMutSig_SigGenesFile ( inName, outName ):
	...
	try:
	    ...
	    if ( pValue <= 0.01 ):
	    	    ...
	    	    ( tokenList[1], tokenList[3], tokenList[4], tokenList[5],
	    ...
	    if ( pValue <= 0.01 ):
	        fhOut.write ( "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % \
	    	    ( tokenList[0], tokenList[1], tokenList[2], tokenList[5],
...
def parseMutSigFiles ( lastDir, outDir ):
	...
	## if ( d2Name.find("Mutation_Significance.Level_4")>0  or  d2Name.find("MutSigNozzleReportCV.Level_4")>0 ):
	if ( d2Name.find("Mutation_Significance.Level_4")>0  or  d2Name.find("MutSigNozzleReport1.5.Level_4")>0 ):
		...
		## the first file we are interested in is the patients.counts_and_rates file ...
    	        if ( fName.endswith("patients.counts_and_rates.txt") ):
		    inName = fName
    	            print " >>> got one !!! ", fName
                    numGot += 1
		    outName = outDir + getLastBit(d2Name) + ".patients.counts_and_rates.tsv"
		...
		## we don't really want it anyway, Brady's mutation bits are better
		if ( 0 ):
		    if ( fName.endswith("sample_sig_gene_table.txt") ):
		        inName = fName
		        outName = outDir + getLastBit(d2Name) + ".sample_sig_gene_table.tsv"
...
def makeGisticFeatureName ( namePrefix, roiName, cytoName, widePeakString, peakString, \
    ...
    if ( threshString == "Actual Copy Change Given" ):
        featName = "N:CNVR:%s:%s:%d:%d::%s_Gistic_ROI_r_" % ( cytoName.strip(), chrName, peakBounds[0], peakBounds[1], namePrefix )
    elif ( threshString.startswith("0:") ):
        featName = "C:CNVR:%s:%s:%d:%d::%s_Gistic_ROI_d_" % ( cytoName.strip(), chrName, peakBounds[0], peakBounds[1], namePrefix )
    else:
    ...
    if ( 0 ): print " returning : <%s> " % featName
    return ( featName )
...
def parseGistic_LesionsFile ( inName, outName ):
    ...
    goodData = 1
    if ( hdrTokens[0] != "Unique Name" ): goodData = 0
    if ( hdrTokens[1] != "Descriptor" ): goodData = 0
    if ( hdrTokens[2] != "Wide Peak Limits" ): goodData = 0
    if ( hdrTokens[3] != "Peak Limits" ): goodData = 0
    if ( hdrTokens[4] != "Region Limits" ): goodData = 0
    if ( hdrTokens[5] != "q values" ): goodData = 0
    if ( hdrTokens[6] != "Residual q values after removing segments shared with higher peaks" ): goodData = 0
    if ( hdrTokens[7] != "Broad or Focal" ): goodData = 0
    if ( not goodData ):
        print " ERROR ??? unexpected hdrTokens ??? ", len(hdrTokens)
        print hdrTokens
	sys.exit(-1)
	...
	if ( len(tokenList) != numTokens ):
	    ...
	    featName = makeGisticFeatureName ( namePrefix, roiName, cytoName, widePeakString, \
...
def parseGistic_BroadArmValuesFile ( inName, outName ):
    ...
    ## sanity check the header tokens a bit ...
    goodData = 1
    if ( hdrTokens[0] != "Chromosome Arm" ): goodData = 0
    if ( not goodData ):
        print " ERROR ??? unexpected hdrTokens ??? ", len(hdrTokens)
        print hdrTokens
	sys.exit(-1)
	...
	if ( len(tokenList) != numTokens ):
	    ...
	    ## create two features per arm ... a 'continuous' feature and a 'discrete' feature

	    ## first the continuous or "_r" feature:
	    featName = "N:CNVR:" + armName + ":chr" + chrName + ":%d:%d::%s_GisticArm_r" % ( startPos, stopPos, namePrefix )
	    ...
	    featName = "C:CNVR:" + armName + ":chr" + chrName + ":%d:%d::%s_GisticArm_d" % ( startPos, stopPos, namePrefix )
	    armDict[featName] = numpy.zeros ( numSamples )
	    armList += [ featName ]
	    for ii in range(numSamples):
	...
	if ( d2Name.find("CopyNumber_Gistic2.Level_4") > 0 ):
		...
		if ( fName.find("all_lesions") >= 0 ):
		        ...
		        inName = fName
    	                print " >>> got one !!! ", fName
                        numGot += 1
		        outName = outDir + getLastBit(d2Name) + ".all_lesions.tsv"
		        parseGistic_LesionsFile ( inName, outName )

		## the next file we are interested in is the broad_values_by_arm file ...
		if ( fName.endswith("broad_values_by_arm.txt") ):
		    inName = fName
    	            print " >>> got one !!! ", fName
                    numGot += 1
		    outName = outDir + getLastBit(d2Name) + ".broad_values_by_arm.tsv"
		    parseGistic_BroadArmValuesFile ( inName, outName )
                ...
                if ( fName.endswith("all_data_by_genes.txt") ):
                    ...
                    print " >>> got one !!! ", fName
                    numGot += 1
                    outName = outDir + getLastBit(d2Name) + ".all_data_by_genes.txt"
...
def buildSampleWhiteList ( lastDir, outDir ):
    ...
    outName = outDir + "gdac.broadinstitute.org_" + dateString + ".patientList.txt"
    fh = file ( outName, 'w' )
    for aSample in cleanPatientList:
        fh.write ( "%s\n" % aSample )
    fh.close()

    outName = outDir + "gdac.broadinstitute.org_" + dateString + ".sampleList.txt"
    fh = file ( outName, 'w' )
    for aSample in cleanSampleList:
        fh.write ( "%s\n" % aSample )
    fh.close()
...
if __name__ == "__main__":
    ...
    cancerDirNames = [ 'blca', 'brca', 'cesc', 'cntl', 'coad', 'dlbc', 'esca', 'gbm', 'hnsc', 'kirc', \
