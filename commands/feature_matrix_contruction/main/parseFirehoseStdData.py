...
def parseClinicalDataFiles ( lastDir, outDir, zCancer, subsetName, suffixString ):
    ...
    if ( subsetName == "" ):
        ...
        expectedFileName = zCancer.upper() + "-" + subsetName + "clin.merged.picked.txt"
    	        ...
    	        for fName in d3.files():
                    ...
                    if ( fName.endswith(expectedFileName) ):
    	                if ( 1 ):
                            ...
                            outFile = zCancer + ".firehose__clin_merged." + suffixString + ".tsv"
                            try:
                            ...
                            ## --------------------------------------
                            ## ok, time to parse the input file and write the output file
                            ...
                            ## we need to turn these patient barcodes into tumor sample barcodes
                            numPatients = len(barcodeList)
                            tumorBarcodeList = []
                            for iP in range(numPatients):
                                patientBarcode = barcodeList[iP]
                                tumorBarcode = miscTCGA.get_tumor_barcode ( patientBarcode )
                            ...
                            ## we need to make sure that we are generating unique feature names
                            uniqNames = []
                            ...
                            for aLine in fh:
                                ...
                                if ( aLine.startswith("Composite Element REF") ): continue
                                if ( aLine.find("dccupload") >= 0 ): continue
                                ...
                                ## and at the data ...
                                dataVec = tokenList[1:]
                                ( dataType, dataCard, fracNA, dataVec ) = lookAtData ( dataVec )
                                print featName, dataType, dataCard, fracNA

                                ( newFeatName1, newFeatName2 ) = makeFeatureName ( featName, dataType )
	...
	if ( d2Name.find("Merge_methylation__") > 0 ):
		    ...
		    if ( fName.find("methylation__humanmethylation") >= 0 ):
    	                if ( fName.endswith(".data.txt") ):
                            ...
                            if ( subsetName != "" ):
                                if ( fName.find(subsetName) < 0 ): continue
                            ...
                            if ( fName.find("humanmethylation450") > 0 ):
                                outFile = zCancer + ".jhu-usc.edu__humanmethylation450__methylation." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                                ...
                                outFile = zCancer + ".jhu-usc.edu__humanmethylation27__methylation." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                            ...
                            metaData = getMetaDataInfo ( "/titan/cancerregulome9/TCGA/firehose/metadata/meth.probes.15oct13.txt" )
		    ...
		    if ( fName.find("mirnaseq__") >= 0 ):
    	                if ( fName.endswith(".data.txt") ):
                            ...
                            if ( fName.find("illuminahiseq_mirnaseq__bcgsc") > 0 ):
                                outFile = zCancer + ".bcgsc.ca__illuminahiseq_mirnaseq__mirnaseq." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                                ...
                                outFile = zCancer + ".bcgsc.ca__illuminaga_mirnaseq__mirnaseq." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
		    ...
		    if ( fName.find("miRseq_mature") >= 0 ):
    	                if ( fName.endswith("_log2.txt") ):
                            ...
                            if ( fName.find("miRseq_mature_PKM") > 0 ):
                                ...
                                ## because it won't break the subsequent pipeline steps ...
                                outFile = zCancer + ".bcgsc.ca__illuminahiseq_mirnaseq__mirnaseq." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                            ...
                            while not done:
                                ...
                                aFeature = "N:MIRN:" + aSplit[0] + ":::::" + aSplit[1]
                                print aFeature
		    ...
		    if ( fName.find("rppa_core__") >= 0 ):
    	                if ( fName.endswith(".data.txt") ):
                            ...
                            if ( fName.find("protein_exp__mda_rppa_core") > 0 ):
                                outFile = zCancer + ".mdanderson.org__mda_rppa_core__protein_exp." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                            ...
                            for kk in range(iCol,numTok,numCol):
                                ...
                                barcode = miscTCGA.fixTCGAbarcode ( hdr1Tokens[kk], zCancer )
                                outLine += "\t" + barcode
                            fhOut.write ( "%s\n" % outLine )
                            ...
                            while not done:
                                ...
                                aFeature = "N:RPPA:" + geneName + ":::::" + aList[0]
                                ## print aFeature
...
def parseRNAseqDataFiles ( lastDir, outDir, zCancer, subsetName, suffixString ):
	...
	if ( d2Name.find("Merge_rnaseq") > 0 ):
            ...
            ## whereas if it comes from BCGSC then we just need to check for gene expression data
		    ...
		    if ( fName.find(".rnaseq") >= 0 ):
    	                if ( fName.endswith(".data.txt") ):
                            ...
                            if ( fName.find("illuminahiseq_rnaseqv2") > 0 ):
                                outFile = zCancer + ".unc.edu__illuminahiseq_rnaseqv2__rnaseqv2." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                                    platformName = "IlluminaHiSeq_RNASeqV2"
                                    ## in this file the only column is called "normalized_count"
                                    numCol = 1 
                                    iCol = 1
                                except:
                                    print " ERROR ??? failed to open output file ??? "
                                ...
                                outFile = zCancer + ".bcgsc.ca__illuminaga_rnaseq__rnaseq." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                                    platformName = "IlluminaGA_RNASeq"
                                    ## NOTE: the 3 columns are raw_counts, median_length_normalized, and RPKM
                                    numCol = 3
                                    iCol = 3
                                except:
                                    print " ERROR ??? failed to open output file ??? "
                                ...
                                outFile = zCancer + ".bcgsc.ca__illuminahiseq_rnaseq__rnaseq." + suffixString + ".tsv"
                                try:
                                    print " opening output file : ", outFile
                                    fhOut = file ( outFile, 'w' )
                            ...
                            while not done:
                                ...
                                ## so then it gets split into [ 'Em:AC008101.5', '?_calculated' ]

                                geneTokens = aList[0].split('|')

                                ## fiddle around with these gene tokens a little bit ...
                                if ( len(geneTokens) < 2 ):
                                ...
                                if ( geneTokens[0] == "?" ):
                                    if ( len(geneTokens) < 2 ):
                                        ...
                                        print geneTokens
                                        sys.exit(-1)
                                    geneTokens[-1] = cleanString ( geneTokens[-1] )
                                    ## aFeature = "N:GEXP:" + geneTokens[-1] + ":::::" + geneTokens[-1]
                                    aFeature = "N:GEXP:" + geneTokens[-1] + ":::::" 
                                else:
                                    geneTokens[0]  = cleanString ( geneTokens[0] )
                                    if ( len(geneTokens) == 1 ):
            ...
            if ( d2Name.find("segmented_scna_minus_germline_cnv_hg19__seg.Level_3.") > 0 ):
                ...
                for fName in d3.files():
                    ...
                    if ( fName.endswith("hg19__seg.seg.txt") ):
                        ...
                        inputFile = fName
                        outputFile = zCancer + ".broad.mit.edu__genome_wide_snp_6__snp." + suffixString + ".tsv"
                        ## cmdString = "python $TCGAFMP_ROOT_DIR/main/snp_firehose_Level3_matrix.py --include " + inputFile + " " + outputFile
                        cmdString = "python $TCGAFMP_ROOT_DIR/main/snp_firehose_Level3_matrix.py " + inputFile + " " + outputFile
...
if __name__ == "__main__":
    ...
    if ( 1 ):
		...
		sys.exit(-1)
            ...
            if ( len(sys.argv) >= 4 ):
                ...
                ## if we are not told where to get the stddata, then get the most recent ...
            ...
            if ( len(sys.argv) == 5 ):
                ...
                if ( not subsetName.endswith(".") ): subsetName += "."
            else:
                subsetName = ""
	...
	lastDir = getCancerDir ( topDir, zCancer )
        ...
        if ( 1 ):
            parseClinicalDataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )

        ## next we parse the methylation data file(s)
        if ( 1 ):
            parseMethylationDataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )

        ## and then the microRNA data file(s)
        if ( 1 ):
            ## parseMicroRNADataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )
            parseMatureMicroRNADataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )

        ## and then RPPA data file(s)
        if ( 1 ):
            parseRPPADataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )
        ...
        if ( 1 ):
            parseRNAseqDataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )

        ## and finally the copy-number resegmentation ...
        if ( 1 ):
            parseSNP6dataFiles ( lastDir, outDir, zCancer, subsetName, suffixString )
