            ...
            if ( aToken.startswith("TCGA-") ):
                ...
                if ( 1 ):
                    if ( possibleBarcode not in sampleDict.keys() ):
                        ...
                        sampleDict[possibleBarcode] = [ ( setName, platformName, dateString ) ]
                    else:
                        addFlag = 1
                        for aTuple in sampleDict[possibleBarcode]:
                            fullMatch = 1
                            if ( setName != aTuple[0] ): fullMatch = 0
                            if ( platformName != aTuple[1] ): fullMatch = 0
                            if ( dateString != aTuple[2] ): fullMatch = 0
                            if ( fullMatch ): addFlag = 0
                        if ( addFlag ):
                            sampleDict[possibleBarcode] += [ ( setName, platformName, dateString ) ]
...
def buildSampleWhiteList ( lastDir, outDir ):
    ...
    outName = outDir + "gdac.broadinstitute.org__" + runName + "__" + dateString + ".sampleList.txt"
    fh = file ( outName, 'w' )
    ...
    outName = outDir + "gdac.broadinstitute.org_" + runName + "__" + dateString + ".sampleList.txt"
    fh = file ( outName, 'w' )
    for aSample in sampleDict:
        fh.write ( "%s\n" % aSample )
    fh.close()
