...
def newContentOK ( oldString, newString, strName ):
    ...
    ## try removing special characters from the 'new' string and see if it matches
...
def removeSpecialChars ( aLabel ):
    ...
    if ( bLabel != aLabel ):
	if ( 0 ): print " in removeSpecialChars : <%s> <%s> " % ( aLabel, bLabel )
...
def walk ( parent, xmlDict, xmlFlags ):
	...
	## each node is either an ELEMENT_NODE or a TEXT_NODE
		...
		if ( len(strContent) > 0 ):
		    ...
		    if ( strName.find("_followup_barcode") > 0 ): continue
		    if ( strName.find("_followup_uuid") > 0 ): continue
		    if ( strName.startswith("rx:") ): continue
		    if ( strName.startswith("rad:") ): continue
		    if ( strName.endswith("_text") ): continue
		    if ( strName.endswith("_notes") ): continue
			...
			if ( str(xmlDict[strName]) != str(strContent) ):
			    ...
			    ## ------------------------------------------------------------
			    ## HERE ... we should decide whether to keep new content or not
				...
				## there are certain types of keys that we want to keep no matter what ...
		    ...
		    if ( insertNew ):
			    ...
			    ## if that doesn't work, then it must be a STRING
				    ...
				    if ( (strName.find("barcode")<0) and (strName.find("uuid")<0) ):
				        fixContent = removeSpecialChars ( strContent )
...
if __name__ == "__main__":
	...
	topDir = "/titan/cancerregulome11/TCGA/repositories/" + snapshotName + "/public/tumor/" + zCancer + "/bcr"

	## get the contents of the bcr directory and loop over the subdirectories which
	## represent the specific BCR's (e.g. intgen.org and nationwidechildrens.org)
	d1 = path.path ( topDir )
	print d1
	print len(d1.dirs())
	for d1Name in d1.dirs():

	    ## NEW: 08mar13 ... intgen.org should be completely phased out by now ...
	    if ( d1Name.find("intgen.org") >= 0 ): continue

	    print '    ', d1Name

	    ## under the BCR-specific directory is a "bio" and then "clin" (or at least it should)
		...
		if ( d2Name.find("Level_1") >= 0 ):
			...
			if ( fName.find("clinical") < 0 ): continue
		        print '            ', fName
    ...
    fhHdr1.write ( "@END_TIMESTAMP %s %d features %d patients" % ( datetime.today().isoformat(), len(bestKeyOrder), numClinicalFiles ) )
    fhHdr2.write ( "@END_TIMESTAMP %s %d features %d patients" % ( datetime.today().isoformat(), len(bestKeyOrder), numClinicalFiles ) )
