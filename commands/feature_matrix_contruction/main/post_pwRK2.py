...
if __name__=="__main__":
    ...
    if ( iOne == -1 ):
	    ...
	    for jF in range(iF,numFeatTypes):
	        oFile = sDir + "post_proc_all." + shortTypes[iF] + "." + shortTypes[jF] + ".tmp"
	        ## print oFile
    ...
    while not done:
        ...
        ## we either just keep incrementing iFeat until we don't find any more files ...
	...
	try:
        ...
        ## or if we are definitely only processing one, we are done after one loop
        if ( iFeat == iOne ):
            done = 1
        ...
        sleepTime = 0
        while not seemsOK:
            if ( sleepTime==0 ): print " testing whether <%s> seems ok or not ... " % inFile
            aLine = fhIn.readline()
            if ( len(aLine) > 2 ):
                ...
                if ( sleepTime > 30 ):
                    if ( iFeat == iOne ):
                        ...
                        seemsOK = 1
                if ( sleepTime==0 ): print " empty file ??? ", inFile
                time.sleep(1)
                sleepTime += 1
            fhIn.close()
            fhIn = file ( inFile, 'r' )
	...
	if ( iFeat%10000 == 0 ): print " feature # ", iFeat
	...
	for aLine in fhIn:
	    ...
	    if ( num < 10 ): dqFlag = 1
	    if ( rhoString != "NA" ):
	        if ( (logP > 299) and (abs(float(rhoString))<0.1) ): dqFlag = 1
	    ...
	    if ( dqFlag ):
                numDQ += 1
		fhLog.write ( " not writing out: %s\t%s\t%s\t%s\t%d\t%.1f\t%.1f\t%.1f\t%d\t%.1f\t%d\t%.1f\t%9d\n" % \
			...
			( aFeat, bFeat, rhoString, num, logP, logBonf, blogP, nA, pA, nB, pB, abDist )
	        else:
		    outString += "%s\t%s\t%s\t%d\t%.1f\t%.1f\t%.1f\t%d\t%.1f\t%d\t%.1f\t%9d\n" % \
			( bFeat, aFeat, rhoString, num, logP, logBonf, blogP, nB, pB, nA, pA, abDist )

	        fhOut.write ( "%s" % outString )
		if ( iOne == -1 ):
	            subFh[iF][jF].write ( "%s" % outString )
	        numLinesOut += 1
	        if ( numLinesOut%1000000 == 0 ): print "     number of output pairs : ", numLinesOut
