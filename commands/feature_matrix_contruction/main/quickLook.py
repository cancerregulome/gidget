...
if __name__ == "__main__":
	...
	bLine = fh.readline()
	bLine = bLine.strip()
	## each bTokens will have a feature name, followed by a list of feature values
	bTokens = bLine.split('\t')
	...
	for iCol in range(1,numCol):
	    ...
	    if ( featName.find("unknown") >= 0 ): continue
	    for iRow in range(numRow):
    ...
    for iCol in range(1,numCol):
	    ...
	    if ( (dataMatrix[iCol][iRow]=="NA") or (dataMatrix[iCol][iRow]=="na") or (dataMatrix[iCol][iRow]=="null") ):
	        numNA += 1
		numNAbyRow[iRow] += 1
		if ( maxNA < numNAbyRow[iRow] ): maxNA = numNAbyRow[iRow]
		if ( haveDataTypes ):
    ...
    for iRow in range(numRow):
		...
		try:
			...
			nz_byDataType[dataType] += 1
		        ...
		        if ( fVal < min_byDataType[dataType] ): min_byDataType[dataType] = fVal
		        if ( fVal > max_byDataType[dataType] ): max_byDataType[dataType] = fVal
		except:
		    doNothing = 1
