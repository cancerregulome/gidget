...
if __name__=="__main__":
    ...
    ## note that we must either have an integer value in 'one' or else 'all' must be TRUE
    print args
    indexString = ''
    if ( not args.all ):
        if ( args.one==None ):
	...
	iOne = index
	cmdString = "python $TCGAFMP_ROOT_DIR/main/post_pwRK2.py %s %s %d" % ( tmpDir, tsvFile, iOne )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
	print status, output
	...
	## out something that looks like the output from runPWPV
	cmdString = "python $TCGAFMP_ROOT_DIR/main/post_pwRK2.py %s %s" % ( tmpDir, tsvFile )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
