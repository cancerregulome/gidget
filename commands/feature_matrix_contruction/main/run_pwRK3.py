...
def getFeatureIndex ( indexString, featureMatrixFile ):
    ...
    if ( len(matchList) == 1 ): return ( indexList[0] )
...
def getIndexRanges ( tsvFile, aType ):
    ...
    typeList = [ "CLIN", "CNVR", "GEXP", "GNAB", "METH", "MIRN", "RPPA", "SAMP" ]
...
def tuplesOverlap ( iTuple, jTuple ):
    ...
    if ( iTuple[0] >= jTuple[1] ): return ( 0 )
    if ( jTuple[0] >= iTuple[1] ): return ( 0 )
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser ( description='Create runlist for pairwise' )
    parser.add_argument ( '--min-ct-cell', '-minct', action='store', default=5, type=int )
    parser.add_argument ( '--min-mx-cell', '-minmx', action='store', default=5, type=int )
    parser.add_argument ( '--min-samples', '-M', action='store', default=20, type=int )
    parser.add_argument ( '--pvalue', '-p', action='store', default=0.000001, type=float )
    ...
    parser.add_argument ( '--verbosity', '-v', action='store', default=0, type=int )
    ...
    parser.add_argument ( '--useBC', '-B', action='store', default=99, type=float )
    ...
    ## get the tsv feature matrix file and also the number of features it contains
    ...
    ## we need to pre-process the tsv file (unless it appears to have already been done)
	...
	for index in range (numFeat-1):
        ...
        for iTuple in iRanges1:
	        ...
	        cmdString += " --pvalue %g --min-ct-cell %d --min-mx-cell %d --min-samples %d" \
                ...
                ## here we need to adjust things so that we don't do the same comparison twice ...
	    		...
	    		% ( iTuple[0], iTuple[1]+1, jStart, jStop, binFile, outName )
                    ...
                    ## print "     handling NONoverlapping tuples ", iTuple, jTuple
	            cmdString += " --outer %d:%d:1 --inner %d:%d:1  %s  %s " \
	...
	iOne = index
	cmdString = "python $TCGAFMP_ROOT_DIR/main/post_pwRK2.py %s %s %d %g" % ( tmpDir, tsvFile, iOne, args.useBC )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
	print status, output
	...
	cmdString = "mv %s/%d.all.pwpv.sort %s.%d.all.pwpv.sort" % ( tmpDir, iOne, tsvFile[:-4], iOne )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
	print status, output
	...
	## out something that looks like the output from runPWPV
	cmdString = "python $TCGAFMP_ROOT_DIR/main/post_pwRK2.py %s %s -1 %g" % ( tmpDir, tsvFile, args.useBC )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
        print " STATUS : ", status
        print " OUTPUT : ", output
	...
	## and now we move the files that we want to keep ...
        if ( args.byType ):
            cmdString = "uniq %s/post_proc_all.short.sort.mapped.noPathway > %s.%s.%s.pwpv.forRE" % \
                ( tmpDir, tsvFile[:-4], cleanString(args.type1), cleanString(args.type2) )
        else:
            cmdString = "mv %s/post_proc_all.short.sort.mapped.noPathway %s.pwpv.forRE" % ( tmpDir, tsvFile[:-4] )
        print " < %s > " % cmdString
        ( status, output ) = commands.getstatusoutput ( cmdString )
	...
	## and now we move the files that we want to keep ...
	cmdString = "mv %s/post_proc_all.NGEXP.NGEXP.tmp.sort.top1M %s.pwpv.NGEXP.NGEXP.top1M" % ( tmpDir, tsvFile[:-4] )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )

	cmdString = "mv %s/post_proc_all.NGEXP.NGEXP.tmp.sort %s.pwpv.NGEXP.NGEXP.all" % ( tmpDir, tsvFile[:-4] )
	print " < %s > " % cmdString
	( status, output ) = commands.getstatusoutput ( cmdString )
