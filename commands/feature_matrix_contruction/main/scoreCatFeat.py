...
def order(x):
    if ( debugFlag ): print " in FUNC order() ... "
    L = len(x)
    rangeL = range(L)
...
def rank(x):
    if ( debugFlag ): print " in FUNC rank() ... "
    L = len(x)
    ordering = order(x)
    ranks = [0] * L
    for i in range(L):
...
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
## new 28-jan-2013 from http://stackoverflow.com/questions/3071415/efficient-method-to-calculate-the-rank-vector-of-a-list-in-python
...
def getTypeRanks ( aType, tmpDir, indexList, featList ):
	...
	## first, open the previously generated pairwise file for this index (feature)
	...
	for aLine in fh:
	    ...
	    if ( aLine.find(aType) >= 0 ):
	        tokenList = aLine.split('\t')
		## check that the # of samples involved in the pairwise test was at least 20 ...
		if ( int(tokenList[4]) < 20 ): continue
...
if __name__=="__main__":
    ...
    parser.add_argument ( '--min-samples', '-M', action='store', default=30, type=int )
	...
	## pTmp : # of p-values considered for each feature name (this is a dictionary with the feature name as the key)
...
####		pCounts[outNames[kR]] = pTmp[outNames[kR]]
	...
	## pCounts{}    : this keeps track of how many p-values contributed to this rank/score
	...
	aName = nameList[kR]
	## print iR, jR, kR, nameList[kR], sumScores[kR], ( float(sumScores[kR])/float(maxScore) )
    ...
    if ( numOut == 0 ):
        fh.write ( "## there were no significant associations between clinical/sample features and molecular features \n" )
    fh.close()
