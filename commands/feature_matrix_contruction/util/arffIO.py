...
def noData ( aValue ):
    ...
    ## print aValue
    try:
        if ( aValue.upper() == "NA" ): return ( 1 )
        if ( aValue.upper() == "NAN" ): return ( 1 )
    except:
        try:
            fVal = float ( aValue )
            return ( 0 )
...
def writeARFF ( allClinDict, bestKeyOrder, progName, outName ):
    ...
    for ii in range(len(bestKeyOrder)):
        ...
        ( keyTypes[ii], nCount, nNA, nCard, labelLists[ii], labelCounts[ii] ) = miscClin.lookAtKey ( allClinDict[aKey] )
	## print aKey, useFlags[ii], keyTypes[ii], nCount, nNA, nCard
	if ( useFlags[ii] ):
	    if ( keyTypes[ii] == 'NUMERIC' ): numNumeric += 1
	    if ( keyTypes[ii] == 'NOMINAL' ): numNominal += 1
	else:
	    numSkip += 1
	...
	if ( useFlags[ii] ):
		...
		for jj in range(len(allClinDict[aKey])):
			...
			if ( minVal > xVal ): minVal = xVal
			if ( maxVal < xVal ): maxVal = xVal
		    except:
		        doNothing = 1
    ...
    for kk in range(numK):
		...
		if ( noData(allClinDict[aKey][kk]) ):
		    ...
		    if ( keyTypes[ii] == 'NUMERIC' ):
		        ...
		        try:
		           outLine += ( '"%s", ' % allClinDict[aKey][kk].upper() )
			except:
			   outLine += ( '"%s", ' % str(allClinDict[aKey][kk]) )
