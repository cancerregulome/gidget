...
def discretize ( keyVec, keyType, numBins=16, printFlag=0 ):
	...
	for ii in range(nVal):
	    aVal = keyVec[ii]
	    if ( aVal != "NA" ):
	        if ( aVal not in valList ): valList += [ aVal ]
	        tmpV[kk] = aVal
		kk += 1
	tmpV = tmpV[:kk]
	tmpV.sort()
...
def calcProbDist ( iVec, iCard ):
    ...
    if ( abs(1.-checkSum) > 1.e-06 ):
        if ( debugFlag ): print "     normalizing pX ... ", checkSum
        for ix in range(numBins):
	    try:
                pX[ix] /= checkSum
	    except:
...
def calcProbDist2 ( iVec, iCard, jVec, jCard ):
    ...
    if ( abs(1.-checkSum) > 1.e-06 ):
        if ( debugFlag ): print "     normalizing pXY ... ", checkSum
        for ix in range(iCard):
            for iy in range(jCard):
                pXY[ix][iy] /= checkSum
...
def calcProbDist3 ( iVec, iCard, jVec, jCard, kVec, kCard ):
    ...
    if ( abs(1.-checkSum) > 1.e-06 ):
	if ( debugFlag ): print "     normalizing pXYZ ... ", checkSum
	for ix in range(iCard):
	    for iy in range(jCard):
		for iz in range(kCard):
...
def calcCondEntropy ( xVec, xCard, yVec, yCard ):
    ...
    for ix in range(len(pX)):
        for iy in range(len(pY)):
            ...
            if ( pXY[ix][iy] < 1.e-24 ): continue
            try:
                HofXgivenY -= pXY[ix][iy] * miscMath.log2 ( pXY[ix][iy] / pY[iy] )
            except:
...
def calcCondEntropy2 ( xVec, xCard, yVec, yCard, zVec, zCard ):
	...
	for iy in range(len(pY)):
	    for iz in range(len(pZ)):
	        ## if the joint probability is zero, then skip this [ix,iy,iz] bin ...
		if ( pXYZ[ix][iy][iz] < 1.e-24 ): continue
	        try:
		    HofXgivenYZ -= pXYZ[ix][iy][iz] * miscMath.log2 ( pXYZ[ix][iy][iz] / pYZ[iy][iz] )
	        except:
...
def lookAtClinDict ( allClinDict ):
    ...
    for ii in range(len(allKeys)):
        ...
        allStr = 1
	allNum = 1
        if ( len(allClinDict[aKey]) == 0 ): continue
	for kk in range(numClin):
	    if ( allClinDict[aKey][kk] != "NA" ):
			...
			sys.exit(-1)
		    if ( oldStr != newStr ):
		        ## print " oldStr = <%s>  newStr = <%s> " % ( oldStr, newStr )
...
def filterClinDict ( allClinDict, \
	...
	## labelCounts is the number of counts associated with each label in labelLists (for NUMERIC data it is [])
        ( keyTypes[ii], nCounts[ii], naCounts[ii], cardCounts[ii], labelLists[ii], labelCounts[ii] ) = lookAtKey ( allClinDict[aKey] )

	naFrac = float(naCounts[ii])/float(nCounts[ii])
	allNAfrac += [ naFrac ]

	## we test the NA fraction, as well as the min/max class sizes for NOMINAL data ...

	if ( keyTypes[ii] == "NOMINAL" ):
	    if ( naFrac <= categorical_naFracThresh ):
	        keepFlags[ii] = 1
	    else:
	    	tossString += " naFrac = %.3f > %.3f " % ( naFrac, categorical_naFracThresh )
	    ...
	    if ( minFrac < classSize_minFracThresh ):
	        keepFlags[ii] = 0
	        tossString += " minFrac = %.3f < %.3f " % ( minFrac, classSize_minFracThresh )
	
	if ( keyTypes[ii] == "NUMERIC" ):
	    if ( naFrac <= numerical_naFracThresh ):
	        keepFlags[ii] = 1
	    else:
	    	tossString += " naFrac = %.3f > %.3f " % ( naFrac, numerical_naFracThresh )
	    ## if ( keepFlags[ii] ):
    ...
    if ( 1 ):
	    ...
	    print "     in entropy computation loop ... ", ii, numKeys
	        ...
	        if ( not keepFlags[jj] ): continue
	        bKey = allKeys[jj]
	        if ( bKey == "bcr_patient_barcode" ): continue
	        ...
	        ( tmpI1, card1, qFlag1 ) = discretize ( allClinDict[aKey], keyTypes[ii] )
	        ( tmpI2, card2, qFlag2 ) = discretize ( allClinDict[bKey], keyTypes[jj] )
	        ...
	        if ( len(tmpI1)==0 or len(tmpI2)==0 ): continue
	        if ( max(tmpI1)==0 or max(tmpI2)==0 ): continue
		    ...
		    if ( nUse1 > 0 ): print " tmpI1 : ", tmpI1
		    if ( nUse2 > 0 ): print " tmpI2 : ", tmpI2
		    sys.exit(-1)
	            ...
	            if ( abs(HofX-HofY) < 0.01 ):
		            ...
		            if ( na1 == na2 ):
		                ...
		                print " --> would have removed %s because %s has the same information, but the number of NAs is not the same (%f,%f,%f) (%d,%d) " % \
		    		    	    ( bKey, aKey, HofX, HofXgivenY, xyMI, na2, na1 )
			        ## check to see if they are identical when not NA ... ??? HERE
			        allSame = 1
			        for oo in range(len(allClinDict[aKey])):
			            if ( allClinDict[aKey][oo] != "NA" ):
				        if ( allClinDict[bKey][oo] != "NA" ):
				            if ( allClinDict[aKey][oo] != allClinDict[bKey][oo] ): allSame = 0
			        if ( allSame ):
			            if ( na1 > na2 ):
...
def chooseWeakestLink ( ii, jj, kk, pmiMatrix ):
    ...
    if ( miAB < min(miBC,miAC) ): return ( ii, jj )
    if ( miBC < min(miAB,miAC) ): return ( jj, kk )
    if ( miAC < min(miAB,miBC) ): return ( ii, kk )
...
def pairwiseMI ( allClinDict, outFilename='' ):
	...
	if ( keyTypes[ii] == "NOMINAL" ):
	    ( tmpI1, card1, qFlag1 ) = discretize ( allClinDict[aKey], keyTypes[ii], 16, 1 )
	...
	### HACK
	### if ( aKey < "N:CNVR:14q32.32_GisticROI_d:chr14:102454171:102471808:" ): continue
	    ...
	    if ( 0 ):
	        aTest = "|" + aKey + ")"
	        if ( bKey.find(aTest) >= 0 ): continue
	        bTest = "|" + bKey + ")"
	        if ( aKey.find(bTest) >= 0 ): continue

	    ## don't bother with the very high cardinality clusterings ...
	    if ( 0 ):
	        if ( aKey.find("_K5") > 0 ): continue
	        if ( aKey.find("_K6") > 0 ): continue
	        if ( aKey.find("_K7") > 0 ): continue
	        if ( aKey.find("_K8") > 0 ): continue
	        if ( bKey.find("_K5") > 0 ): continue
	        if ( bKey.find("_K6") > 0 ): continue
	        if ( bKey.find("_K7") > 0 ): continue
	        if ( bKey.find("_K8") > 0 ): continue
	    ...
	    ( rhoP, rhoS ) = computeRhos ( allClinDict[aKey], keyTypes[ii], allClinDict[bKey], keyTypes[jj] )
	    ...
	    ## try a few different levels of discretization to see if it affects the MI ???
	    if ( 1 ):
	        if ( (qFlag1+qFlag2) > 0 ):
		    ...
		    best_MI = 0.
		    tryN = [ 2, 4, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256 ]
		    ...
		    for i1 in try1:
			    ...
			    if ( i1 > 0 ):
			        if ( len(allClinDict[aKey]) < i1 ): continue

			    if ( i2 > 0 ):
			        if ( len(allClinDict[bKey]) < i2 ): continue
			    ...
			    ## print " trying different numbers of bins ... ", i1, card1, i2, card2

			    if ( i1 > 0 ):
			        ( a_tmpI1, a_card1, a_qFlag1 ) = discretize ( allClinDict[aKey], keyTypes[ii], i1 )
			    else:
			        ( a_tmpI1, a_card1, a_qFlag1 ) = discretize ( allClinDict[aKey], keyTypes[ii] )
			    if ( i2 > 0 ):
			        ( a_tmpI2, a_card2, a_qFlag2 ) = discretize ( allClinDict[bKey], keyTypes[jj], i2 )
			    else:
			        ( a_tmpI2, a_card2, a_qFlag2 ) = discretize ( allClinDict[bKey], keyTypes[jj] )
			    ( a_tmpI1, a_tmpI2, a_na1, a_na2 ) = removeZeros ( a_tmpI1, a_tmpI2 )

			    ## if the discretization results in any unused bins, then
			    ## we assume that that is too many levels of discretization ...
			    stop1 = 0
			    stop2 = 0
			    ...
			    ( a_HofX, a_maxHX ) = calcEntropy ( a_tmpI1, a_card1 )
			    ( a_HofY, a_maxHY ) = calcEntropy ( a_tmpI2, a_card2 )
			    a_HofXgivenY = calcCondEntropy ( a_tmpI1, a_card1, a_tmpI2, a_card2 )
				...
				## keep1 = i1
				## keep2 = i2
				## print "         --> best so far ", i1, i2, best_MI
		...
		( p_MI, p_rho ) = rand2x2.getSignif ( n00, n01, n10, n11, xyMI, rhoP, 100 )
		print p_MI, p_rho

		if ( (p_MI+p_rho) < 0.05 ):
		    print "     --> REFINING ... "
		    ( p_MI, p_rho ) = rand2x2.getSignif ( n00, n01, n10, n11, xyMI, rhoP, 1000 )
		    print p_MI, p_rho

		if ( (p_MI+p_rho) < 0.005 ):
		    print "         --> REFINING "
		    ( p_MI, p_rho ) = rand2x2.getSignif ( n00, n01, n10, n11, xyMI, rhoP, 10000 )
		    print p_MI, p_rho

		if ( (p_MI+p_rho) < -0.0005 ):
		    print "         --> REFINING "
		    ( p_MI, p_rho ) = rand2x2.getSignif ( n00, n01, n10, n11, xyMI, rhoP, 100000 )
		    print p_MI, p_rho
		...
		if ( 0 ):
		    ...
		    ( p_MI, p_rho ) = rand2x2.permutationTest ( tmpI1, tmpI2, xyMI, rhoP, 100 )

		if ( (p_MI+p_rho) < 0.05 ):
		    print "     --> REFINING ... "
		    ( p_MI, p_rho ) = rand2x2.permutationTest ( tmpI1, tmpI2, xyMI, rhoP, 1000 )
		    print p_MI, p_rho

		if ( (p_MI+p_rho) < 0.005 ):
		    print "         --> REFINING "
		    ( p_MI, p_rho ) = rand2x2.permutationTest ( tmpI1, tmpI2, xyMI, rhoP, 10000 )
		    print p_MI, p_rho

		if ( (p_MI+p_rho) < -0.0005 ):
		    print "         --> REFINING "
		    ( p_MI, p_rho ) = rand2x2.permutationTest ( tmpI1, tmpI2, xyMI, rhoP, 100000 )
		    print p_MI, p_rho
	    ...
	    if ( xyMI > 0.0 ):
			...
			( miFrac, HofX, maxHX, HofY, maxHY, HofXgivenY, HofX, xyMI, min(HofX,HofY), aKey, bKey )
		if ( writeFlag ):
		    if ( HofX <= HofY ):
		        fh.write ( "%6.3f\t%d\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%8.5f\t%8.5f\t%8.5f\t%s\t%s\n" % \
    ...
    for ii in range(numKeys):
	    ...
	    if ( pmiMatrix[ii][jj] == 0.0 ): continue
	    for kk in range(numKeys):
	        if ( pmiMatrix[ii][kk] == 0.0 ): continue
		if ( pmiMatrix[jj][kk] == 0.0 ): continue
...
def removeDuplicateKeys ( allClinDict ):
	...
	for jj in range(ii+1,len(allKeys)):
		...
		## so now we want to collapse the data and eliminate the redundant key ...
		removeFlags[jj] = 1
		for kk in range(numClin):
...
def removeConstantKeys ( allClinDict ):
    ...
    magicStrings = [ "disease_code", "gender", "days_to_initial", "days_to_death", "icd_", 
...
def removeUninformativeKeys ( allClinDict ):
    ...
    magicStrings = [ "detection_method", "tissue_source_site", "lab_proc", "_dcc_",
		     "procedure_name", "handbook_edition", "batch_number", "_method",
		     "icd_o_3_site", "ospective_collection" ]

    ## new list of 'magic' strings with handbook_edition and batch_number removed (09nov12)
    magicStrings = [ "detection_method", "tissue_source_site", "lab_proc", "_dcc_",
