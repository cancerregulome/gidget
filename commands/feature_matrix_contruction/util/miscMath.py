...
def make_window ( wLen, wType ):
	...
	return ( w )
        ...
        for ii in range(wLen):
            ...
            phi2 = 2. * phi1
            phi3 = 3. * phi1
            w[ii] = 0.355768 - ( 0.487396 * math.cos(phi1) ) + ( 0.144232 * math.cos(phi2) ) - ( 0.012604 * math.cos(phi3) )
        return ( w )
        ...
        for ii in range(wLen):
            ...
            phi2 = 2. * phi1
            phi3 = 3. * phi1
            w[ii] = 0.3635819 - ( 0.4891775 * math.cos(phi1) ) + ( 0.1365995 * math.cos(phi2) ) - ( 0.0106411 * math.cos(phi3) )
        return ( w )
...
def linear_regression ( xVals, yVals ):
    ...
    # if we have essentially zero slope (ySS=0) then we might get problems here ...
    try:
...
def slidingCorrelation ( x, y, kMax ):
    ...
    ## print '         from slidingCorrelation : ', dpMaxPos, bestKpos, dpMaxNeg, bestKneg
...
def computeMatthewsCorrCoeff ( numTP, numTN, numFP, numFN ):
    ...
    b = math.sqrt ( float(numTP+numFP) * float(numTP+numFN) * float(numTN+numFP) * float(numTN+numFN) )
    try:
	return ( (a/b) )
    except:
	return ( -1 )
