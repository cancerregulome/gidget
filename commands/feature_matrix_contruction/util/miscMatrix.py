...
def statsByCol ( matrixFilename ):
	...
	if ( len(tokenList) == numCols ):
		...
		if ( fVal < minVals[iCol] ): minVals[iCol] = fVal
		if ( fVal > maxVals[iCol] ): maxVals[iCol] = fVal
		sum1[iCol] += ( fVal )
		sum2[iCol] += ( fVal * fVal )
	    iRow += 1
	else:
...
def oneDhist ( matrixFilename, iX, deltaX=-1, xMin=-1, xMax=-1, nSkip=0, printFlag=1 ):
	...
	if ( len(tokenList)<= iX ):
	    ...
	    if ( tokenList[iX] == "NaN" ): tokenList[iX] = "NA"
	    try:
	        xCol[iRow] = float ( tokenList[iX] )
    ...
    if ( xMin == -1 ): xMin = xCol.min()
    if ( xMax == -1 ): xMax = xCol.max()
    xMean = xCol.mean()
    xSigma = xCol.std()
	...
	print ' calling getHistogramInfo ... ', xMin, xMax, xSigma
        ( xMinHist, xMaxHist, deltaX, numBins ) = getHistogramInfo ( xMin, xMax, xSigma )
	print xMinHist, xMaxHist, deltaX, numBins
    else:
	print ' calling getHistogramInfo2 ... ', xMin, xMax, deltaX
        ( xMinHist, xMaxHist, deltaX, numBins ) = getHistogramInfo2 ( xMin, xMax, deltaX )
	print xMinHist, xMaxHist, deltaX, numBins
    ...
    iStartBin = 0
    while ( xHist[iStartBin] == 0 ): iStartBin += 1

    iStopBin = numBins - 1
    while ( xHist[iStopBin] == 0 ): iStopBin -= 1
	...
	if ( 1 ):
		...
		if ( xNormInv != -999999999. ):
			...
			print ' %5d  %14.10f  %12d  %14.10f  %14.10f  %14.10f ' % \
				( iBin, xVal, xHist[iBin], curFrac, cumFrac, xNormInv )
	...
	if ( uHeight < 1.e-9 and curFrac < 1.e-9 ):
	    ...
	    doNothing = 1
            ...
            KLdist += curFrac * math.log(curFrac/uHeight)  +  uHeight * math.log(uHeight/curFrac)
...
def oneDcondHist ( matrixFilename, iX, bitVec, deltaX=-1, xMin=-1, xMax=-1, nSkip=0, printFlag=1 ):
    ...
    ## first we'd like to find the first and last non-zero bins ...
    iStartBin = 0
    while ( xHist0[iStartBin]==0 and xHist1[iStartBin]==0 ): iStartBin += 1

    iStopBin = numBins - 1
    while ( xHist0[iStopBin]==0 and xHist1[iStopBin]==0 ): iStopBin -= 1
	...
	if ( 1 ):
		...
		outLine += ' %12d  %14.8f  %14.8f ' % ( xHist0[iBin], curFrac0, cumFrac0 )
		outLine += ' %12d  %14.8f  %14.8f ' % ( xHist1[iBin], curFrac1, cumFrac1 )
...
def twoDhist ( matrixFilename, iX, iY, \
    ...
    if ( xMin == '' ): xMin = xCol.min()
    if ( xMax == '' ): xMax = xCol.max()
    xMean = xCol.mean()
    xSigma = xCol.std()
    print '         ', xMin, xMax, xMean, xSigma

    if ( yMin == '' ): yMin = yCol.min()
    if ( yMax == '' ): yMax = yCol.max()
    yMean = yCol.mean()
    ySigma = yCol.std()
    print '         ', yMin, yMax, yMean, ySigma

    if ( deltaX == '' ):
        ( xMinHist, xMaxHist, deltaX, numXbins ) = getHistogramInfo ( xMin, xMax, xSigma )
    else:
    ...
    if ( deltaY == '' ):
        ( yMinHist, yMaxHist, deltaY, numYbins ) = getHistogramInfo ( yMin, yMax, ySigma )
    else:
