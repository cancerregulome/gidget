...
def parseTCGAbarcode ( barcode ):
    ...
    if ( bLen > 15 ): vial = barcode[15]	## single character
    if ( bLen > 18 ): portion = barcode[17:19]	## two digits
    if ( bLen > 19 ): analyte = barcode[19]	## single character
    if ( bLen > 24 ): plate = barcode[21:25]	## four char/digit plate id
    if ( bLen > 27 ): center = barcode[26:28]	## two digit center id
...
def get_uuid2barcode_dict():
    ...
    print " reading metadata file for UUID to barcode mapping "

    fileName = "/titan/cancerregulome11/TCGA/repositories/uuids/metadata.current.txt"
    ...
    if ( refreshFlag ):
	cmdString = "/titan/cancerregulome11/TCGA/repositories/uuids/get_metadata.sh"
	print " "
	...
	if ( not looks_like_uuid(uuid) ): continue
	if ( not looks_like_barcode(barcode) ): continue
	...
	if ( 1 ):
	    ...
	    if ( uuid not in uuid2barcode_dict.keys() ):
	        uuid2barcode_dict[uuid] = barcode
	    else:
    ...
    print " done reading UUID to barcode mapping file (%d UUIDs and %d patients) " % ( len(uuid2barcode_dict), len(patient_dict) )
