...
def _prettyPrint2 (aDict):
    ...
    for iStart in startKeys:
        ...
        if ( len(stopKeys) != 1 ):
            print aDict
            raise ValueError(" ERROR ??? in _prettyPrint2 ... how can there by multiple stop keys ??? ")
        iStop = stopKeys[0]
        try:
            ...
            mess = "ERROR in _prettyPrint2 ???  startKeys: %s contents for current startKey: %i %s stopKeys: %s number of stopKeys: %i\n\t" % \
                (str(startKeys), iStart, str(aDict[iStart]), str(stopKeys), len(aDict[iStart]))
...
def _removeZerosNAs(diffvec):
    ## new, shaves off a bit of the over all time --mm 2012-08-06 (note: depends on diffvec being >= 0, otherwise use abs(diffvec))
    if ( len(diffvec) < 1 ):
        raise ValueError('in _removeZerosNAs() ... zero length vector ???')

    notZerosNAs = numpy.logical_and(diffvec > NEAR_ZERO, diffvec < abs(NA_VALUE)-1)
    diffvec_nz = diffvec[notZerosNAs]
    if len(diffvec_nz) < 1:
        _prettyPrint(diffvec)
        raise ValueError("ERROR ???  in _removeZerosNAs() ... %i %i" % (len(diffvec), len(diffvec_nz)))
    return diffvec_nz
    ...
    if ( n2 < 10 ):
        _prettyPrint(diffvec)
        raise ValueError("ERROR ???  in _removeZerosNAs() ... %i %i %i %i" % (len(diffvec), n1, n0, n2))
...
def _computeFirstDiffVec(cnvsum):
    ...
    na = numpy.logical_or(abs(cnvsum[0:-1]) == abs(NA_VALUE), abs(cnvsum[1:]) == abs(NA_VALUE))
    ...
    if ( 0 ):
        if ( max(diffvec[notNA]) > 1000 ):
            ...
            raise ValueError("ERROR ??? how did we get this value %f for max value in _computeFirstDiffVec??? " % max(diffvec[notNA]))
    ...
    if ( 0 ):
        if ( maxDiff > 1000 ):
            ...
            for ii in range(len(diffvec)):
                print ii, cnvsum[ii+1], cnvsum[ii], diffvec[ii]
            raise ValueError("ERROR ??? how did we get this value %f for max value in _computeFirstDiffVec??? " % maxDiff)
...
def _validate(start, stopPlus1, length):
    ...
    if ( start < 0 ):
        ## print "ERROR in _copyInto ??? %i %i %i %s" % (len(cnvvec), start, stopPlus1, str(value))
        raise ValueError("ERROR in _copyInto ??? %i %i %i" % (length, start, stopPlus1))
        start = 0
    if ( start >= (length-1) ):
        ## print " ERROR in _copyInto ??? ", len(cnvvec), start, stopPlus1, value
        raise ValueError("ERROR in _copyInto ??? %i %i %i" % (length, start, stopPlus1))
        start = length - 1
    if ( stopPlus1 < 1 ):
        ## print " ERROR in _copyInto ??? ", len(cnvvec), start, stopPlus1, value
...
#            cnvvec = _copyInto(cnvvec, startcoord, stopcoord + 1, tup[valueIndex])
            ...
            ## print " "
            ## print " calling copyInto ... ", startcoord, data[sampleIndex][start]
            if startcoord < prevAdjStopIndex:
                raise ValueError('need to add support for large overlapping segments. overlap: %i' % startcoord < prevAdjStopIndex)
            ...
            if maxchrcoord - 1 == stopcoord:
                # this is to stay compatible with the previous code (which always makes the last segment = NA_VALUE(???NOT SURE WHY?) --mm)
                stopcoord -= 1
            ...
            if endStartOverlap and abs(values[-2]) < abs(values[-1]):
                ...
                while 0 == ranges[-2]:
                    ...
                    # print 'removing range and value (%.4f) before [%.4f,%i]' % (values[-2], values[-1], ranges[-1])
                    del ranges[-2:-1]
                    del values[-2:-1]
                ...
                if abs(values[-2]) < abs(values[-1]):
                    ...
                    ranges[-1] += 1
                    if 0 > ranges[-2]:
                        raise ValueError('problem creating array for sample index %s: %i %s %i %s.  adjusted range is negative' % (barcodeList[jS], len(values), values, len(ranges), ranges))
        ...
        try:
            dataTemp[jS] = numpy.repeat(numpy.array(values), numpy.array(ranges))
            dataZero[jS] = numpy.repeat(numpy.array(zeroedValues), numpy.array(ranges))
            #mask = dataZero == NA_VALUE
            #dataZero[mask] = 0
        except Exception as e:
            traceback.print_exc(10)
            raise ValueError('problem creating array for sample index %s: %i %s %i %s' % (barcodeList[jS], len(values), values, len(ranges), ranges), e)
...
def _resegmentChr(chrName, dataTuple):
    ...
    ##  chr   barcode                          start      stop      value,   start      stop      value, ...} ...} ...}
    ...
    cnvsum, numSamples, dataTemp, dataZero = _setupSegments(chrName, chr2data, chr2maxcoord, barcodeList, barcode2index, steplength)
    ...
    ## bins for this chrosome, and the second is the # of bins with non-zero slope)
    ...
    lastcoord = -1
    segList = []
    cutoff_indices = numpy.where(numpy.logical_and(diffvec >= cutoff_hi, diffvec < abs(NA_VALUE) - 1))[0]
    if cutoff_indices[len(cutoff_indices) - 1] != len(diffvec) - 1:
        cutoff_indices = numpy.append(cutoff_indices, len(diffvec) - 1)

    beforeNA_indices = numpy.where(numpy.logical_and(diffvec[0:-1] < abs(NA_VALUE), diffvec[1:] == abs(NA_VALUE)))[0] + 1
    # don't use a quick sort because the arrays above will be somewhat sorted already
    cutoff_indices = numpy.sort(numpy.unique(numpy.insert(cutoff_indices, len(cutoff_indices), beforeNA_indices)), None, 'Heapsort')
    ...
    for index in range(len(cutoff_indices)):
        ...
        navalues_indices = numpy.where(diffvec[lastcoord+1:cutoff_index] > abs(NA_VALUE) - 1)[0]
        ...
        ## print "             difference is significant", diffvec[cutoff_index], startcoord, stopcoord, startpos, stoppos

        ## print " F5  looping over barcodes ... ", barcodeList[:5], " ... ", barcodeList[-5:]

        segList += ['N:CNVR::chr' + chrName + ':' + str(startpos) + ':' + str(stoppos) + '::']
        newMatrix[:, index] = _getLocalArrayMean(dataTemp[:, startcoord:(stopcoord + 1)], dataZero[:, startcoord:(stopcoord + 1)])[0]
...
def _resegmentChromosomes(chrNames, chr2data, chr2maxcoord, sampleList, steplength, cutFrac):
    ...
    segList = []
    dataTuple = (chr2data, chr2maxcoord, barcodeList, barcode2index, steplength, cutFrac)
