...
def writeAttributeHeader ( dataD, bestKeyOrder, fh, sortRowFlag=0 ):
	...
	for aKey in bestKeyOrder:
	    ...
	    ( keyType, nCount, naCount, cardCount, labelList, labelCount ) = miscClin.lookAtKey ( dataD[aKey] )
	    ## print keyType, nCount, naCount, cardCount, labelList, labelCount

	    outLine += "\t%d\t%d\t%d\t%s" % ( nCount, naCount, (nCount-naCount), keyType )
		...
		if ( maxV > minV ):
		    ...
		    outLine += "\tCONTINUOUS"
		    if ( isInt ):
		        outLine += "\t%d\t%d\t%d\t%d\t%d\t%5.2f" % ( int(minV), int(maxV), int(medianV), int(meanV), int(sigmaV), skewV )
		    else:
		        outLine += "\t%f\t%f\t%f\t%f\t%f\t%5.2f" % ( minV, maxV, medianV, meanV, sigmaV, skewV )
		    ## outLine += "\t%f
		else:
		    outLine += "\tCONSTANT"
		    if ( isInt ):
...
def writeTSV_clinical ( allClinDict, bestKeyOrder, outName ):
	...
	p1 = allClinDict[bestKeyOrder[0]][k1]
	    ...
	    if ( keepPatient[k1] == 0 ): continue
	    if ( keepPatient[k2] == 0 ): continue
		...
		if ( sameFlag == 1 ):
			...
			if ( allClinDict["batch_number"][k1] > allClinDict["batch_number"][k2] ):
			    ...
			    ## if the batch numbers appear to be the same, check the BCR ...
	...
	if ( not keepPatient[kk] ): continue
        aKey = bestKeyOrder[0]
        outLine = "%s" % ( replaceBlanks ( str ( allClinDict[aKey][kk] ), '_' ) )
	for ii in range(1,len(bestKeyOrder)):
	    aKey = bestKeyOrder[ii]
            if ( len(allClinDict[aKey]) != numClin ): continue
	    try:
...
def writeTSV_clinicalFlipNumeric ( allClinDict, bestKeyOrder, outName ):
	...
	if ( keyType == "NOMINAL" ):
		...
		for jj in range(numClin):
		    curLabel = replaceBlanks ( str(allClinDict[aKey][jj]).upper(), '_' )
		    outLine += "\t%s" % curLabel
		fh.write ( "%s\n" % outLine )
		...
		## should we call a "categorical" feature with cardinality 2 binary ???
		if ( 0 ):
		    if ( nCard == 2 ):
		        if ( featureName[0] != "B" ):
		...
		if ( 0 ):
		    ...
		    print " NOT remapping from labels to integers ... ", labelList, featureName
        	    outLine = "%s" % ( replaceBlanks ( featureName, '_' ) )
		    for jj in range(numClin):
		        curLabel = replaceBlanks ( allClinDict[aKey][jj].upper(), '_' )
		        outLine += "\t%s" % curLabel
		    fh.write ( "%s\n" % outLine )
...
def buildPath ( outFilename ):
    ...
    ## so figre out the directory path name and then create it ...
...
def createDir ( dirName ):
    ...
    i1 = 0
    while ( dirName[i1]=='/' ): i1 += 1
    while ( i1 < len(dirName) ):
        i2 = dirName.find("/",i1)
	if ( i2 <= i1 ): i2 = len(dirName)
	tmpName = dirName[:i2]
...
def getSkipList ( maxNAfrac, dataD, rowColFlag ):
	...
	for aKey in numNA.keys():
	    ...
	    ## print aKey, numNA[aKey], numNot[aKey], numTot, tmpNAfrac, maxNAfrac
...
def addConstFeature ( dataD, newFeatureName, newFeatureValue ):
    ...
    newD['rowLabels'] = dataD['rowLabels']
    newD['colLabels'] = dataD['colLabels']

    newD['rowLabels'] += [ newFeatureName ]
...
def lookAtDataD ( dataD ):
    ...
    if ( len(dataD['colLabels']) != len(dataD['dataMatrix'][0]) ):
        print " ERROR in lookAtDataD ... why do these dimensions not match ??? !!! ", len(dataD['colLabels']), len(dataD['dataMatrix']) 
	sys.exit(-1)
...
def dataMatrix_stats ( dataMatrix, type ):
	...
	for ii in range(numRow):
		...
		if ( curVal != NA_VALUE ):
			...
			## if ( allFloat ): print " --> will skip row #%d " % ( ii+1 )
		        allFloat = 0
	    if ( not allFloat ): continue

	    for jj in range(numCol):
	        curVal = dataMatrix[ii][jj]
		if ( curVal == NA_VALUE ): continue
...
def dataMatrix_quantiles ( dataMatrix, type, nQuant ):
	...
	for ii in range(numRow):
		...
		for iQ in range(nQuant+1):
		    if ( iQ == 0 ): jj = 0
		    elif ( iQ == nQuant ): jj = nCount-1
		    else: jj = int ( float(nCount)*float(iQ)/float(nQuant)  +  0.50 )
...
def dataMatrix_histogram ( dataMatrix, histInfo=(-1,-1,-1,-1) ):
	...
	if ( idel > 0.6 ): delVal = delVal * 0.2
	elif ( idel > 0.4 ): delVal = delVal * 0.4
	elif ( idel > 0.3 ): delVal = delVal * 0.4
	## print delVal
    ...
    iFirst = 0
    while ( hist[iFirst] == 0 ): iFirst += 1
    iLast = numBins - 1
    while ( hist[iLast] == 0 ): iLast -= 1
    ...
    histInfo = ( ( (minVal+float(iFirst)*delVal), (maxVal-(float(numBins-iLast-1)*delVal)), delVal, \
...
def getSampleOverlap ( dataA, dataB ):
	...
	if ( aSample in overlapList ): continue
...
def filter_dataMatrix ( dataD, rmRowList, rmColList ):
    ...
    if ( (numRowDel+numColDel) == 0 ): return ( dataD )
...
def pruneTSV_dataMatrix ( dataD, rowMaxNAfrac, colMaxNAfrac, pruneOrder ):
    ...
    ## print " checking for features and/or samples that have too much missing data ... "
...
def uniqueFeatureLabels ( fLabels, dataMatrix ):
    ...
    for ii in range(len(fLabels)):
        print ii, len(fLabels), fLabels[ii]
        if ( ii in rmRowList ): continue
        aLabel = fLabels[ii]
	nMatch = 0

	for jj in range(ii+1,len(fLabels)):
            if ( jj in rmRowList ): continue
	    bLabel = fLabels[jj]
	    ...
	    if ( aLabel == bLabel ):
                ...
                aVec = dataMatrix[ii]
                bVec = dataMatrix[jj]
                print aLabel, aVec[:10]
                print bLabel, bVec[:10]
                if ( aVec == bVec ):
                    print " identical !!! "
                    rmRowList += [ jj ]
                    numRemove += 1
		    ...
		    try:
		        ...
		        print aLabel
		        sys.exit(-1)
		    newB = bLabel + "%s" % (nMatch+1)
		    fLabels[ii] = newA
		    fLabels[jj] = newB
		    print " <%s> and <%s> renamed to <%s> and <%s> " % ( aLabel, bLabel, newA, newB )
		    numRename += 1
...
def writeTSV_dataMatrix ( dataD, sortRowFlag,  sortColFlag, outFilename ):
    ...
    for ii in range(len(colLabels)):
        aSample = tmpColList[ii]
	for jj in range(len(colLabels)):
	    if ( colLabels[jj] == aSample ):
	        newColOrder[ii] = jj
    ...
    for ii in range(len(rowLabels)):
        aLabel = tmpRowList[ii]
	for jj in range(len(rowLabels)):
	    if ( rowLabels[jj] == aLabel ):
	        newRowOrder[ii] = jj
...
def readTSV ( inName ):
    ...
    if ( len(hdrTokens) < 2 ): return ( {} )

    ## does the first column header look like a barcode / sample ID ???
    isClinical = 0
    firstColHdr = (hdrTokens[0]).lower()
    ## print " in readTSV ... ", firstColHdr
    if ( firstColHdr == "bcr_patient_barcode" ): isClinical = 1
    if ( firstColHdr.find("barcode") >= 0 ): isClinical = 1
    if ( firstColHdr.find("sample") >= 0 ): isClinical = 1
    if ( firstColHdr.find("patient") >= 0 ): isClinical = 1
    if ( firstColHdr.find("tcga_id") >= 0 ): isClinical = 1
    if ( isClinical ):
...
def fixFeatureName ( rowTokens ):
    ...
    fltMutStrings = [ '_binding_delta', '_bound_delta', '_oe_score', '_iarc_freq' ]

    dataTypeList = [ "B", "N", "C" ]
    featTypeList = [ "CLIN", "SAMP", "GNAB", "CNVR", "GEXP", "MIRN", "METH", "PRDM" ]

    if ( len(rowTokens) == 1 ):
        tmpName = rowTokens[0]
	for aString in bitMutStrings:
            if ( tmpName.find(aString) > 0 ): bitMutFlag = 1
	for aString in fltMutStrings:
	    if ( tmpName.find(aString) > 0 ): fltMutFlag = 1
	...
	if ( bitMutFlag ):
	    ii = tmpName.find("_")
	    geneName = tmpName[:ii]
	    newName = "B:GNAB:" + geneName + ":::::" + rowTokens[0][ii+1:] + "_mut"
	    return ( newName )
	elif ( fltMutFlag ):
	    ii = tmpName.find("_")
	    geneName = tmpName[:ii]
	    newName = "N:GNAB:" + geneName + ":::::" + rowTokens[0][ii+1:] + "_mut"
	    return ( newName )
	...
	if ( rowTokens[0] in dataTypeList ):
	    ...
	    if ( len(rowTokens[1]) == 4 ): 
	        ...
	        if ( rowTokens[1] == "GNAB" ): 
		    tmpName = rowTokens[2]
		    if ( tmpName.find("y_n") > 0 ): bitMutFlag = 1
		    if ( tmpName.find("_nonsilent") > 0 ): bitMutFlag = 1
		    if ( tmpName.find("_dna_bin") > 0 ): bitMutFlag = 1
		    if ( tmpName.find("_ns_or_fs") > 0 ): bitMutFlag = 1
		    if ( tmpName.find("_dom_") > 0 ): bitMutFlag = 1
		    if ( bitMutFlag ):
			ii = tmpName.find("_")
			geneName = tmpName[:ii]
			newName = "B:GNAB:" + geneName + ":::::" + rowTokens[0][ii+1:] + "_mut"
		    ...
		    return ( newName )
                    ...
                    newName = rowTokens[0] + ":" + rowTokens[1] + ":" + rowTokens[2] + ":" + rowTokens[3] + ":" + rowTokens[4] + ":" + rowTokens[5] + ":"
                    return ( newName )
...
def readTSV_dataMatrix ( inName ):
    ...
    tsvReader = csv.reader ( open(inName,'rb'), delimiter='\t', quoting=csv.QUOTE_NONE )
    ii = -1
    ...
    ## HACK
    if ( 1 ):
        if ( outDict['dataType'] == "GEXP" ): outDict['dataType'] = "N:GEXP"
        if ( outDict['dataType'] == "METH" ): outDict['dataType'] = "N:METH"
        if ( outDict['dataType'] == "CNVR" ): outDict['dataType'] = "N:CNVR"
...
def readTSV_clinical ( inName ):
    ...
    while not done:
        aLine = fh.readline()
	aLine = aLine.strip()
	## print " readTSV_clinical : %d  %d  aLine = <%s> " % ( ii, len(aLine), aLine )
	if ( len(aLine) == 0 ): 
	    done = 1
	else:
	    try:
	        aLine = aLine.strip()
	        tokenList = aLine.split('\t')
	        ## print "     len(tokenList) = %d (%d) " % ( len(tokenList), numHdrTokens )
	        if ( len(tokenList) == numHdrTokens ):
		        ...
		        ## print " <%s>  %d  %d  " % ( aToken, ii, kk ), tokenList[kk]
		    ii += 1
	        else:
	...
	if ( aKey == '' ): continue
        ...
        allNumeric = 1
	for ii in range(len(allClinDict[aKey])):
	    if ( allClinDict[aKey][ii].upper() == "NA" ): continue
	    if ( allClinDict[aKey][ii].upper() == "NAN" ): continue
	    try:
...
def removeNearlyConstantRows ( tsvData, minFrac ):
	...
	if ( curFrac < minFrac ):
	    ## print " eliminating this row ... ", iRow, tsvData['rowLabels'][iRow], numNA, numZero, numOther
	    numToss += 1
	    skipRowList += [ iRow ]
	else:
	    ## print " KEEPING this row ... \t%d\t%s\t%d\t%d\t%d" % ( iRow, tsvData['rowLabels'][iRow], numNA, numZero, numOther )
	    numKeep += 1
    ...
    if ( skipRowList != [] ): tsvData = tsvIO.filter_dataMatrix ( tsvData, skipRowList, [] )
