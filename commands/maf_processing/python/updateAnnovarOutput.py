        ...
        if not transcripts == "":
            ...
            if len(transcriptList) > 0:
                ...
                for transcript in transcriptList:
                    dnpMatcher = re.match("^(.+:.+:.+):c\.(\d*_*\d+[T,C,G,A]+.*)", transcript) 
                    normalNucleotide = fields[lenFields - 4]
                    if dnpMatcher and not normalNucleotide == "":
                        newTranscript = "%s:%s%s%s" %(dnpMatcher.group(1), "c.", normalNucleotide, dnpMatcher.group(2))
...
def processDNP(annovarNonSNP, mafLine, annotation, mafline2proteinSeqMap):
    ...
    if annovarNonSNP != "" and annotation != "":
        ...
        DNPMatcher = re.match("^([^:]+):([^\s:]+):([^\s:]+):c\.(\d+_\d+)([TCGA]+)(.*)", annotation)
        if DNPMatcher:
            ...
            if key in mafline2proteinSeqMap:
                ...
                for index in range(proteinSeqEndLine - proteinSeqStartLine + 1):
                    translation += linecache.getline(annovarNonSNP, proteinSeqStartLine + index).rstrip("\n")
        ...
        if annotations != "":
            ...
            if len(annotationList) > 0:
                ...
                for annotation in annotationList:
                    transcriptMatcher = re.match("^([^:]+):([^\s:]+):(.*)", annotation)
                    if transcriptMatcher:
                        ...
                        if key in mafline2proteinSeqMap:
                            ...
                            newAnnotation = processDNP(nonSNPCodingChanges, mafLine, annotation, mafline2proteinSeqMap)
                            if newAnnotation == "":
